...
def collect_argument_information(src_dir):
    ...
    for arg_name, setting in settings.items():
        ...
        # Link GetArg/AddArg calls and figure out setting types and default values.
        setting_name = ''.join(word.capitalize() for word in arg_name.replace('?', 'Q').split('-')) + "Setting"
        ...
        for add in setting.adds:
            ...
            for get in setting.gets:
                if add.call.context != get.call.context and (add.call.context != "main" or get.call.context == "test" or add.hidden_args is not None):
                    continue
                if get.add is None:
                    get.add = add
