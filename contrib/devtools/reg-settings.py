...
class AddArg:
    ...
    data_types: dict[DataType | str, SettingType] = field(default_factory=dict)
    includes: set[str] = field(default_factory=set)
...
def parse_calls(file_path: str) -> tuple[list[AddArg], list[GetArg]]:
    ...
    for match in re.finditer(r'(!*)\b(?:((?:\w|\.|->|\(\))+)(\.|->))?(GetArg|GetPathArg|GetIntArg|GetBoolArg|GetArgs|IsArgSet|IsArgNegated)\((.)', content):
        ...
        # Override data_type if templated GetArg<T> called with nonstring T deduced from default value
        if default_arg is not None and function_name == "GetArg":
            for pattern, options in ARG_PATTERNS.items():
                if options.ctype and re.search(pattern, default_arg):
                     data_type = options.ctype
        default_value = (
...
def collect_argument_information(src_dir: str) -> dict[str, Setting]:
    ...
    for arg_name, setting in settings.items():
        ...
        for add in setting.adds:
            ...
            add.data_types[min((isinstance(dt, str), dt) for dt in add.data_types.keys())[1]].primary = True
...
def setting_definitions(add: AddArg) -> tuple[list[str], set[str]]:
    ...
    if add.nonhidden_arg is None:
        ...
        for _, data_type, setting_type in sorted((isinstance(dt, str), dt, st) for dt, st in add.data_types.items()):
            includes.update(setting_type.includes)
            ctype = (data_type if isinstance(data_type, str) else
                      "std::vector<std::string>" if data_type == DataType.STRING_LIST else
...
class PatternOptions:
    ...
    ctype: str | None = None
    """ctype overrides the C++ type used in the setting definition, instead of
    the default type inferred from the DataType (e.g. int64_t for INT). Useful
    when the actual constant has a narrower type like int."""
...
ARG_PATTERNS = {
    ...
    "DEFAULT_HTTP_THREADS": PatternOptions(include_path="httpserver.h", ctype="int"),
    "DEFAULT_HTTP_WORKQUEUE": PatternOptions(include_path="httpserver.h", ctype="int"),
    ...
    "DEFAULT_PRINT_MODIFIED_FEE": PatternOptions(include_path="node/settings.h"),
    "DEFAULT_PRIORITY": PatternOptions(runtime=True),
    "DEFAULT_RPC_REQ_ID": PatternOptions(runtime=True),
    ...
    "DEFAULT_TXOSPENDERINDEX": PatternOptions(include_path="index/txospenderindex.h"),
    ...
    "MAX_CLUSTER_COUNT_LIMIT": PatternOptions(include_path="txgraph.h"),
    "MAX_SCRIPTCHECK_THREADS": PatternOptions(include_path="node/settings.h"),
    ...
    "embedded_asmap_doc": PatternOptions(extern=True),
    ...
    "GetDefaultDBCache\\(\\)": PatternOptions(include_path="node/caches.h", runtime=True),
    ...
    "uint8_t\\{": PatternOptions(ctype="uint8_t"),
    '"auto"': PatternOptions(extern=True),
