    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        if self.use_gui:
            ...
            if sys.platform.startswith("linux") and "XDG_RUNTIME_DIR" not in subp_env:
                # Qt prints a warning to stderr when XDG_RUNTIME_DIR is not set (e.g. in
                # CI environments without a desktop session), which would cause tests to fail
                # due to unexpected stderr output. Point it to the node's datadir, which
                # exists by the time start() is called.
                subp_env["XDG_RUNTIME_DIR"] = str(self.datadir_path)
        if env is not None:
            subp_env.update(env)
