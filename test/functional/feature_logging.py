...
class LoggingTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Test loglevels RPC")
        ...
        result = self.nodes[0].loglevels({'net': 'trace'})
        ...
        result = self.nodes[0].loglevels({'net': 'debug'})
        assert result['net'] == 'debug'

        # Set a category back to info (disable it)
        result = self.nodes[0].loglevels({'net': 'info'})
        assert result['net'] == 'info'
        assert not self.nodes[0].logging()['net']

        # Set all categories to debug using the "all" special key
        result = self.nodes[0].loglevels({'all': 'debug'})
        assert all(v == 'debug' for v in result.values()), f"Expected all debug, got: {result}"
        assert self.nodes[0].logging()['net']
        assert self.nodes[0].logging()['http']

        # "all" is applied first; per-category override takes effect after
        result = self.nodes[0].loglevels({'all': 'info', 'net': 'trace'})
        assert result['net'] == 'trace'
        assert result['http'] == 'info'
        # Same result regardless of dict order
        result = self.nodes[0].loglevels({'net': 'trace', 'all': 'info'})
        assert result['net'] == 'trace'
        assert result['http'] == 'info'

        # Set all categories to trace, then override net back to info
        result = self.nodes[0].loglevels({'all': 'trace', 'net': 'info'})
        assert result['net'] == 'info'
        assert result['http'] == 'trace'

        # Empty dict is a no-op (read-only)
        before = self.nodes[0].loglevels()
        assert self.nodes[0].loglevels({}) == before

        # Invalid category raises an error
        assert_raises_rpc_error(-8, "unknown logging category", self.nodes[0].loglevels, {'notacategory': 'debug'})

        # Invalid level raises an error
        assert_raises_rpc_error(-8, "unknown log level", self.nodes[0].loglevels, {'net': 'verbose'})

        # loglevels and logging RPCs stay consistent
        self.nodes[0].loglevels({'all': 'info', 'rpc': 'debug'})
        logging_result = self.nodes[0].logging()
        levels_result = self.nodes[0].loglevels()
        for cat, active in logging_result.items():
            expected_active = levels_result[cat] != 'info'
            assert active == expected_active, f"Inconsistency for {cat}: logging={active}, loglevels={levels_result[cat]}"
