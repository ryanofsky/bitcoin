...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-4, "Wallet file verification failed. Refusing to load database. Data file '{}' is already loaded.".format(dat), self.nodes[0].loadwallet, wallet_names[0])

        # Fail to load duplicate wallets by different ways (directory and filepath)
        dat = os.path.join(self.options.tmpdir, "node0", "regtest", "wallets", "wallet.dat")
        assert_raises_rpc_error(-4, "Wallet file verification failed. Refusing to load database. Data file '{}' is already loaded.".format(dat), self.nodes[0].loadwallet, 'wallet.dat')
