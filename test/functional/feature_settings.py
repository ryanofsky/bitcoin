...
class SettingsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        node.assert_start_raises_init_error(expected_msg='Error: Unable to parse settings file', match=ErrorMatch.PARTIAL_REGEX)

        # Test invalid json object
        with settings.open("w") as fp:
            fp.write('"string"')
        node.assert_start_raises_init_error(expected_msg='settings.json is not in expected key-value format', match=ErrorMatch.PARTIAL_REGEX)

        # Test invalid settings file containing duplicate keys
        with settings.open("w") as fp:
            fp.write('{"key": 1, "key": 2}')
        node.assert_start_raises_init_error(expected_msg='settings.json has duplicate key key', match=ErrorMatch.PARTIAL_REGEX)

        # Test invalid settings file is ignore with command line -nosettings
