    ...
    def run_mining_test(self):
        ...
        async def async_routine():
            ...
            async with AsyncExitStack() as stack:
                ...
                template2 = await wait_and_do(wait_next_template(template, stack, ctx, waitoptions), lambda: self.generate(self.nodes[0], 1))
                ...
                template4 = await wait_and_do(wait_next_template(template2, stack, ctx, waitoptions), lambda: miniwallet.send_self_transfer(fee_rate=10, from_node=self.nodes[0]))
                ...
                self.log.debug("Wait for another, get one after increase in fees in the mempool")
                template6 = await wait_and_do(wait_next_template(template5, stack, ctx, waitoptions), lambda: miniwallet.send_self_transfer(fee_rate=10, from_node=self.nodes[0]))
