...
class IPCInterfaceTest(BitcoinTestFramework):
    ...
    def run_unclean_disconnect_test(self):
        """Test behavior of disconnecting during an IPC call that returns a
        non-null interface pointer when it returns. Currently this behavior
        causes a crash as reported
        https://github.com/bitcoin/bitcoin/issues/34250, but a followup will
        ...
        async def async_routine():
            ...
            waitoptions.timeout = timeout
            waitoptions.feeThreshold = 1
            with self.nodes[0].assert_debug_log(expected_msgs=["BlockTemplate.waitNext", "IPC server post request"]):
                promise = template.waitNext(ctx, waitoptions)
                await asyncio.sleep(0.1)
            disconnected_log_check.enter_context(self.nodes[0].assert_debug_log(expected_msgs=["IPC server: socket disconnected"], timeout=1))

        asyncio.run(capnp.run(async_routine()))

        # Wait for socket disconnected log message, then generate a block to
        # cause the waitNext() call return a new template. This will cause a
        ...
        self.nodes[0].wait_until_stopped(expected_ret_code=-6, expected_stderr="bitcoin-node: ipc/libmultiprocess/include/mp/proxy.h:59: EventLoop &mp::EventLoopRef::operator*() const: Assertion `m_loop' failed.")
        self.start_node(0)

    def run_thread_busy_test(self):
        """Test behavior when sending multiple calls to same server thread which
        used to cause a crash as reported
        ...
        async def async_routine():
            ...
            # Make multiple waitnext calls where first will start to execute,
            # second will be posted waiting to execute, and third will fail to
            # execute because the execution thread is busy.
            with self.nodes[0].assert_debug_log(expected_msgs=["BlockTemplate.waitNext", "IPC server post request"]):
