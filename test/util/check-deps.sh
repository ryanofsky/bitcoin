...
# is currently part of the node libary. interface_ui should just be part of the
...
usage() {
   echo "Usage: $(basename "${BASH_SOURCE[0]}") SRC_DIRECTORY"
}
...
check_disallowed() {
    ...
    while read symbol; do
        local dst_obj=$(obj_names "$symbol" "${temp_dir}/${dst}_exports.txt")
...
# Check arguments.
if [ "$#" = 0 ]; then
    SOURCE_DIR="$(dirname "${BASH_SOURCE[0]}")/../../src"
elif [ "$#" = 1 ]; then
    SOURCE_DIR="$1"
else
