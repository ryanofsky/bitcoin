...
public:
    PeerManagerImpl(CConnman& connman, AddrMan& addrman,
                    ...
                    CTxMemPool& pool, BCLog::Logger& logger, Options opts);
...
std::unique_ptr<PeerManager> PeerManager::make(CConnman& connman, AddrMan& addrman,
                                               ...
                                               CTxMemPool& pool, BCLog::Logger& logger, Options opts)
{
    return std::make_unique<PeerManagerImpl>(connman, addrman, banman, chainman, pool, logger, opts);
}
      ...
      m_opts{opts}
{
    ...
    if (opts.reconcile_txs) {
        m_txreconciliation = std::make_unique<TxReconciliationTracker>(TXRECONCILIATION_VERSION, logger);
    }
}
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if (!pfrom.IsInboundConn()) {
            ...
            AddTimeData(m_log.logger, pfrom.addr, nTimeOffset);
        }
    ...
    if (msg_type == NetMsgType::VERACK) {
        ...
        if (!pfrom.IsInboundConn() || m_log.logger.WillLogCategoryLevel(BCLog::NET, BCLog::Level::Debug)) {
            const auto mapped_as{m_connman.GetMappedAS(pfrom.addr)};
    ...
    if (msg_type == NetMsgType::SENDTXRCNCL) {
        if (!m_txreconciliation) {
            LogDebug(m_log, "sendtxrcncl from peer=%d ignored, as our node does not have txreconciliation enabled\n", pfrom.GetId());
            return;
        }

        if (pfrom.fSuccessfullyConnected) {
            LogDebug(m_log, "sendtxrcncl received after verack from peer=%d; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (RejectIncomingTxs(pfrom)) {
            LogDebug(m_log, "sendtxrcncl received from peer=%d to which we indicated no tx relay; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        const auto* tx_relay = peer->GetTxRelay();
        if (!tx_relay || !WITH_LOCK(tx_relay->m_bloom_filter_mutex, return tx_relay->m_relay_txs)) {
            LogDebug(m_log, "sendtxrcncl received from peer=%d which indicated no tx relay to us; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        case ReconciliationRegisterResult::NOT_FOUND:
            LogDebug(m_log, "Ignore unexpected txreconciliation signal from peer=%d\n", pfrom.GetId());
            break;
        case ReconciliationRegisterResult::SUCCESS:
            break;
        case ReconciliationRegisterResult::ALREADY_REGISTERED:
            LogDebug(m_log, "txreconciliation protocol violation from peer=%d (sendtxrcncl received from already registered peer); disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        case ReconciliationRegisterResult::PROTOCOL_VIOLATION:
            LogDebug(m_log, "txreconciliation protocol violation from peer=%d; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        return;
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (received_new_header) {
            LogInfo(m_log, "Saw new cmpctblock header hash=%s peer=%d\n",
