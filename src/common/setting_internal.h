...
struct SettingTraitsBase
{
    ...
    static bool Get(const common::SettingsValue& setting, setting_t& out)
    {
        if constexpr (!std::is_same_v<decltype(get_fn), std::nullptr_t>) {
            ...
            // successfully retrieved as any type.
...
template<auto V>
auto format(common::internal::Constant<V> fmt, const auto&... args)
{
    constexpr util::ConstevalFormatString<sizeof...(args)> f{V.value};
    return strprintf(f, args...);
}
} // namespace tinyformat
