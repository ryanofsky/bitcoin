...
{
    return args.IsArgSet("-?") || args.IsArgSet("-h") || HelpSetting::Get(args) || HelpDebugSetting::Get(args);
}
...
{
    const auto options = TestSetting::Get(args).value_or(std::vector<std::string>{});
...
{
    const fs::path datadir{DatadirSettingPath::Get(args).value_or(fs::path{})};
