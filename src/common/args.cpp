                                                  ...
                                                  unsigned int flags, std::string& error)
{
    ...
    if (key.negated) {
        ...
        if (IsTypedArg(flags)) {
            if (value && *value != "1") {
                // If argument is typed, only allow negation with no value or
                // with literal "1" value. Avoid calling InterpretBool and
                // interpreteding other values which could be ambiguous.
