...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        if (!CheckDataDirOption(args)) {
            return ConfigError{ConfigStatus::FAILED, strprintf(_("Specified data directory \"%s\" does not exist."), DatadirSetting::Get(args).value_or(""))};
        }
        ...
        const fs::path orig_config_path{AbsPathForConfigVal(args, ConfSettingPath::Get(args).value_or(BITCOIN_CONF_FILENAME), /*net_specific=*/false)};
        ...
        if (fs::exists(base_config_path) && !fs::equivalent(orig_config_path, base_config_path)) {
            const std::string cli_config_path = ConfSetting::Get(args).value_or("");
