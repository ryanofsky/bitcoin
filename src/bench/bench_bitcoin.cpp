...
||||||| parent of 534f9713c9a4 (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
        args.asymptote = parseAsymptote(argsman.GetArg("-asymptote", ""));
        args.is_list_only = argsman.GetBoolArg("-list", false);
        args.min_time = std::chrono::milliseconds(argsman.GetIntArg("-min-time", DEFAULT_MIN_TIME_MS));
        args.output_csv = argsman.GetPathArg("-output-csv");
...
>>>>>>> 534f9713c9a4 (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
