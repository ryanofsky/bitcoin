...
||||||| parent of 47e30b40eca0 (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
    argsman.AddArg("-acceptnonstdtxn", strprintf("Relay and mine \"non-standard\" transactions (test networks only; default: %u)", DEFAULT_ACCEPT_NON_STD_TXN), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-incrementalrelayfee=<amt>", strprintf("Fee rate (in %s/kvB) used to define cost of relay, used for mempool limiting and replacement policy. (default: %s)", CURRENCY_UNIT, FormatMoney(DEFAULT_INCREMENTAL_RELAY_FEE)), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-dustrelayfee=<amt>", strprintf("Fee rate (in %s/kvB) used to define dust, the value of an output such that it will cost more than its value in fees at this fee rate to spend it. (default: %s)", CURRENCY_UNIT, FormatMoney(DUST_RELAY_TX_FEE)), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::NODE_RELAY);
    argsman.AddArg("-acceptstalefeeestimates", strprintf("Read fee estimates even if they are stale (%sdefault: %u) fee estimates are considered stale if they are %s hours old", "regtest only; ", DEFAULT_ACCEPT_STALE_FEE_ESTIMATES, Ticks<std::chrono::hours>(MAX_FILE_AGE)), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
...
>>>>>>> 47e30b40eca0 (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
    ...
    const ArgsManager& args)
{
    ...
    options.check_blocks = CheckblocksSetting::Get(args, DEFAULT_CHECKBLOCKS);
    options.check_level = ChecklevelSetting::Get(args, DEFAULT_CHECKLEVEL);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (DatadirSetting::Get(args) && !DatadirSettingPath::Get(args).is_absolute()) {
...

    {
        ...
        if (AsmapSetting::Get(args)) {
            fs::path asmap_path = AsmapSetting::Get(args, DEFAULT_ASMAP_FILENAME);
    ...
    if (!connOptions.m_use_addrman_outgoing) {
        ...
        if (DnsseedSetting::Get(args) && DnsseedSetting::Get(args, DEFAULT_DNSSEED) && ProxySetting::Get(args)) {
