...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    for (bool fLoaded = false; !fLoaded && !ShutdownRequested(node);) {
        ...
        if (status == node::ChainstateLoadStatus::SUCCESS) {
            ...
            if (chainman.m_blockman.m_have_pruned && options.check_blocks > MIN_BLOCKS_TO_KEEP) {
                LogWarning(chainman.m_blockman.m_log, "pruned datadir may not have more than %d blocks; only checking available blocks\n",
                                  MIN_BLOCKS_TO_KEEP);
            }
    ...
    if (args.GetBoolArg("-txindex", DEFAULT_TXINDEX)) {
        g_txindex = std::make_unique<TxIndex>(logger, interfaces::MakeChain(node), cache_sizes.tx_index, false, fReindex);
    ...
    if (args.GetBoolArg("-coinstatsindex", DEFAULT_COINSTATSINDEX)) {
        g_coin_stats_index = std::make_unique<CoinStatsIndex>(logger, interfaces::MakeChain(node), /*cache_size=*/0, false, fReindex);
