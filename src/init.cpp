...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    MempoolfullrbfSetting::Register(argsman);
...
void InitParameterInteraction(ArgsManager& args)
{
    ...
    std::string proxy_arg = ProxySetting::Get(args).value_or("");
    ...
    if (!ListenSetting::Get(args).value_or(DEFAULT_LISTEN)) {
    ...
    if (ExternalipSetting::Get(args)) {
        // if an explicit public IP is specified, do not try to find others
        if (args.SoftSetBoolArg("-discover", false))
            LogInfo("parameter interaction: -externalip set -> setting -discover=0\n");
    }

    if (BlocksonlySetting::Get(args).value_or(DEFAULT_BLOCKSONLY)) {
    ...
    if (OnlynetSetting::Get(args)) {
        const auto onlynets = OnlynetSetting::Get(args).value_or(std::vector<std::string>{});
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    if (PruneSetting::Get(args).value_or(0)) {
    ...
    if (ForcednsseedSetting::Get(args) && !DnsseedSetting::Get(args).value_or(DEFAULT_DNSSEED)){
        return InitError(_("Cannot set -forcednsseed to true when setting -dnsseed to false."));
    }

    // -bind and -whitebind can't be set when not listening
    size_t nUserBind = BindSetting::Get(args).value_or(std::vector<std::string>{}).size() + WhitebindSetting::Get(args).value_or(std::vector<std::string>{}).size();
    if (nUserBind != 0 && !ListenSetting::Get(args).value_or(DEFAULT_LISTEN)) {
        return InitError(Untranslated("Cannot set -bind or -whitebind together with -listen=0"));
    }

    // if listen=0, then disallow listenonion=1
    if (!ListenSetting::Get(args).value_or(DEFAULT_LISTEN) && ListenonionSetting::Get(args).value_or(DEFAULT_LISTEN_ONION)) {
    ...
    if (BlockmintxfeeSetting::Get(args)) {
        if (!ParseMoney(BlockmintxfeeSetting::Get(args).value_or(""))) {
            return InitError(AmountErrMsg("blockmintxfee", BlockmintxfeeSetting::Get(args).value_or("")));
        }
    }
    ...
    if (TestSetting::Get(args)) {
        if (chainparams.GetChainType() != ChainType::REGTEST) {
            return InitError(Untranslated("-test=<option> can only be used with regtest"));
        }
        const std::vector<std::string> options = TestSetting::Get(args).value_or(std::vector<std::string>{});
    ...
    const ArgsManager& args)
{
    ...
    options.check_blocks = CheckblocksSetting::Get(args).value_or(DEFAULT_CHECKBLOCKS);
    options.check_level = ChecklevelSetting::Get(args).value_or(DEFAULT_CHECKLEVEL);
    options.require_full_verification = CheckblocksSetting::Get(args) || ChecklevelSetting::Get(args);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    auto opt_max_upload = ParseByteUnits(MaxuploadtargetSetting::Get(args).value_or(DEFAULT_MAX_UPLOAD_TARGET), ByteUnit::M);
    if (!opt_max_upload) {
        return InitError(strprintf(_("Unable to parse -maxuploadtarget: '%s'"), MaxuploadtargetSetting::Get(args).value_or("")));
    }
    ...
    if (DatadirSetting::Get(args) && !DatadirSettingPath::Get(args).value_or(fs::path{}).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if bitcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitcoin is started while in a temporary directory.\n",
                  DatadirSetting::Get(args).value_or(""), fs::PathToString(fs::current_path()));
    }
    ...
    fListen = ListenSetting::Get(args).value_or(DEFAULT_LISTEN);
...

    {
        ...
        if (AsmapSetting::Get(args)) {
            fs::path asmap_path = AsmapSetting::Get(args).value_or(DEFAULT_ASMAP_FILENAME);
    ...
    for (const std::string& socket_addr : BindSetting::Get(args).value_or(std::vector<std::string>{})) {
        std::string host_out;
        uint16_t port_out{0};
    ...
    if (OnlynetSetting::Get(args)) {
        ...
        for (const std::string& snet : OnlynetSetting::Get(args).value_or(std::vector<std::string>{})) {
    ...
    if (!CjdnsreachableSetting::Get(args)) {
        if (OnlynetSetting::Get(args) && g_reachable_nets.Contains(NET_CJDNS)) {
            return InitError(
    ...
    std::string proxyArg = ProxySetting::Get(args).value_or("");
    ...
    const bool onlynet_used_with_onion{OnlynetSetting::Get(args) && g_reachable_nets.Contains(NET_ONION)};
    ...
    if (onion_proxy.IsValid()) {
        ...
        const bool listenonion_disabled{!ListenonionSetting::Get(args).value_or(DEFAULT_LISTEN_ONION)};
    ...
    for (const std::string& strAddr : ExternalipSetting::Get(args).value_or(std::vector<std::string>{})) {
    ...
    const uint16_t default_bind_port =
        static_cast<uint16_t>(PortSetting::Get(args).value_or(Params().GetDefaultPort()));
    ...
    for (const std::string& bind_arg : BindSetting::Get(args).value_or(std::vector<std::string>{})) {
    ...
    for (const std::string& strBind : WhitebindSetting::Get(args).value_or(std::vector<std::string>{})) {
    ...
    connOptions.bind_on_any = BindSetting::Get(args).value_or(std::vector<std::string>{}).empty() && WhitebindSetting::Get(args).value_or(std::vector<std::string>{}).empty();

    // Emit a warning if a bad port is given to -port= but only if -bind and -whitebind are not
    // given, because if they are, then -port= is ignored.
    if (connOptions.bind_on_any && PortSetting::Get(args)) {
        const uint16_t port_arg = PortSetting::Get(args).value_or(0);
    ...
    if (ListenonionSetting::Get(args).value_or(DEFAULT_LISTEN_ONION)) {
    ...
    connOptions.vSeedNodes = SeednodeSetting::Get(args).value_or(std::vector<std::string>{});

    // Initiate outbound connections unless connect=0
    connOptions.m_use_addrman_outgoing = !ConnectSetting::Get(args);
    if (!connOptions.m_use_addrman_outgoing) {
        const auto connect = ConnectSetting::Get(args).value_or(std::vector<std::string>{});
        ...
        if (DnsseedSetting::Get(args) && DnsseedSetting::Get(args).value_or(DEFAULT_DNSSEED) && ProxySetting::Get(args)) {
    ...
    if (!i2psam_arg.empty()) {
        ...
        if (OnlynetSetting::Get(args) && g_reachable_nets.Contains(NET_I2P)) {
            return InitError(
