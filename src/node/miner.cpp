...
void ApplyArgsManOptions(const ArgsManager& args, BlockAssembler::Options& options)
{
    ...
    options.nBlockMaxWeight = BlockmaxweightSetting::Get(args).value_or(options.nBlockMaxWeight);
    if (const auto blockmintxfee{BlockmintxfeeSetting::Get(args)}) {
        if (const auto parsed{ParseMoney(*blockmintxfee)}) options.blockMinFeeRate = CFeeRate{*parsed};
    }
    options.print_modified_fee = PrintprioritySetting::Get(args).value_or(options.print_modified_fee);
}
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if (chainparams.MineBlocksOnDemand()) {
        pblock->nVersion = BlockversionSetting::Get(gArgs).value_or(pblock->nVersion);
    }
