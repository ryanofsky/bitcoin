...
{
    mempool_limits.ancestor_count = LimitancestorcountSetting::Get(argsman).value_or(mempool_limits.ancestor_count);

    if (auto vkb = LimitancestorsizeSetting::Get(argsman)) mempool_limits.ancestor_size_vbytes = *vkb * 1'000;

    mempool_limits.descendant_count = LimitdescendantcountSetting::Get(argsman).value_or(mempool_limits.descendant_count);
...
{
    mempool_opts.check_ratio = CheckmempoolSetting::Get(argsman).value_or(mempool_opts.check_ratio);
    ...
    if (IncrementalrelayfeeSetting::Get(argsman)) {
        if (std::optional<CAmount> inc_relay_fee = ParseMoney(IncrementalrelayfeeSetting::Get(argsman).value_or(""))) {
            mempool_opts.incremental_relay_feerate = CFeeRate{inc_relay_fee.value()};
        } else {
            return util::Error{AmountErrMsg("incrementalrelayfee", IncrementalrelayfeeSetting::Get(argsman).value_or(""))};
        }
    }

    if (MinrelaytxfeeSetting::Get(argsman)) {
        if (std::optional<CAmount> min_relay_feerate = ParseMoney(MinrelaytxfeeSetting::Get(argsman).value_or(""))) {
            // High fee check is done afterward in CWallet::Create()
            mempool_opts.min_relay_feerate = CFeeRate{min_relay_feerate.value()};
        } else {
            return util::Error{AmountErrMsg("minrelaytxfee", MinrelaytxfeeSetting::Get(argsman).value_or(""))};
        }
    ...
    if (DustrelayfeeSetting::Get(argsman)) {
        if (std::optional<CAmount> parsed = ParseMoney(DustrelayfeeSetting::Get(argsman).value_or(""))) {
            mempool_opts.dust_relay_feerate = CFeeRate{parsed.value()};
        } else {
            return util::Error{AmountErrMsg("dustrelayfee", DustrelayfeeSetting::Get(argsman).value_or(""))};
        }
    }
    ...
    mempool_opts.full_rbf = MempoolfullrbfSetting::Get(argsman).value_or(mempool_opts.full_rbf);
    if (!mempool_opts.full_rbf) {
        LogInfo("Warning: mempoolfullrbf=0 set but deprecated and will be removed in a future release\n");
    }

    mempool_opts.persist_v1_dat = Persistmempoolv1Setting::Get(argsman).value_or(mempool_opts.persist_v1_dat);
