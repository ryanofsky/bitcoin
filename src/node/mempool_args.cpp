...
||||||| parent of fc2591a35b06 (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
    mempool_limits.ancestor_count = argsman.GetIntArg("-limitancestorcount", mempool_limits.ancestor_count);
=======
    mempool_limits.ancestor_count = LimitAncestorCountSetting::Get(argsman, mempool_limits.ancestor_count);
>>>>>>> fc2591a35b06 (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
...
util::Result<void> ApplyArgsManOptions(const ArgsManager& argsman, const CChainParams& chainparams, MemPoolOptions& mempool_opts)
{
    ...
    if (auto mb = MaxMemPoolSetting::Get(argsman)) {
    ...
    if (const auto arg{IncrementalRelayFeeSetting::Get(argsman)}) {
    ...
    if (const auto arg{MinRelayTxFeeSetting::Get(argsman)}) {
    ...
    if (const auto arg{DustRelayFeeSetting::Get(argsman)}) {
