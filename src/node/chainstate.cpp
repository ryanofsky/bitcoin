...
{
<<<<<<< HEAD
||||||| parent of b21704ed472f (refactor, validation: Return fatal errors from FlushStateToDisk)
    util::Result<InterruptResult, ChainstateLoadError> result;
=======
    FlushResult<InterruptResult, ChainstateLoadError> result;
>>>>>>> b21704ed472f (refactor, validation: Return fatal errors from FlushStateToDisk)
...
||||||| parent of 0f909123ce97 (refactor, blockstorage: Return fatal error from LoadBlockIndex)
    if (!chainman.LoadBlockIndex()) {
        if (chainman.m_interrupt) {
...
=======
    ...
    if (!load_result) {
        ...
        result.Update(Interrupted{});
        return result;
>>>>>>> 0f909123ce97 (refactor, blockstorage: Return fatal error from LoadBlockIndex)
    }
