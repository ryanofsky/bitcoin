...
<<<<<<< HEAD
        if (!FindUndoPos(state, block.nFile, _pos, ::GetSerializeSize(blockundo) + 40)) {
            ...
            return false;
||||||| parent of 4c3622f34c1f (refactor, blockstorage: Return fatal errors from block writes)
        if (!FindUndoPos(state, block.nFile, _pos, ::GetSerializeSize(blockundo) + 40)) {
...
=======
        if (!(FindUndoPos(state, block.nFile, _pos, ::GetSerializeSize(blockundo) + 40) >> result)) {
            ...
            result.Update(util::Error{std::move(error)});
            return result;
>>>>>>> 4c3622f34c1f (refactor, blockstorage: Return fatal errors from block writes)
        }
