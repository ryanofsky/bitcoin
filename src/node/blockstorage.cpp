...
||||||| parent of 0d9b3d98fefe (refactor, blockstorage: Return FlushResult from flush methods)
...
=======
            if (!(FlushUndoFile(_pos.nFile, true) >> result)) {
                ...
                LogPrintLevel(BCLog::BLOCKSTORAGE, BCLog::Level::Warning, "%s\n", warning.original);
                result.AddWarning(std::move(warning));
>>>>>>> 0d9b3d98fefe (refactor, blockstorage: Return FlushResult from flush methods)
            }
