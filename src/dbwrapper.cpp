 ...
 */
static void HandleError(const util::log::Source& log, const leveldb::Status& status)
{
    if (status.ok())
        return;
...
public:
    CBitcoinLevelDBLogger(const util::log::Source& log) : m_log{log} {}
...
static void SetMaxOpenFiles(const util::log::Source& log, leveldb::Options *options) {
...
static leveldb::Options GetOptions(const util::log::Source& log, size_t nCacheSize)
{
