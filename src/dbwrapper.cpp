...
||||||| parent of 9c4fe11544d (refactor: Pass Logger instances to kernel objects)
...
>>>>>>> 9c4fe11544d (refactor: Pass Logger instances to kernel objects)
                return;
            }
            char buffer[500];
...
||||||| parent of 420426fb825 (refactor: Log kernel output to local log instances)
        TryCreateDirectories(params.path);
        LogInfo("Opening LevelDB in %s", fs::PathToString(params.path));
=======
        TryCreateDirectories(params.path);
        LogInfo(m_log, "Opening LevelDB in %s", fs::PathToString(params.path));
>>>>>>> 420426fb825 (refactor: Log kernel output to local log instances)
    }
