...
{
    if (force) {
        m_node.updateSetting("prune", PruneSetting(prune, m_prune_size_gb));
    } else {
...
QVariant OptionsModel::getOption(OptionID option) const
{
    ...
    case SpendZeroConfChange:
        return ToQVariant(m_node.getPersistentSetting("spendzeroconfchange"), true);
#endif
    case DisplayUnit:
        return nDisplayUnit;
    ...
    case Language:
        return ToQVariant(m_node.getPersistentSetting("language"), "");
    ...
    case DatabaseCache:
        return ToQVariant(m_node.getPersistentSetting("dbcache"), (qint64)nDefaultDbCache);
    case ThreadsScriptVerif:
        return ToQVariant(m_node.getPersistentSetting("par"), DEFAULT_SCRIPTCHECK_THREADS);
    case Listen:
        return ToQVariant(m_node.getPersistentSetting("listen"), DEFAULT_LISTEN);
    default:
        return QVariant();
    }
}
...
bool OptionsModel::setOption(OptionID option, const QVariant& value)
{
    ...
    case MapPortUPnP: // core option - can be changed on-the-fly
        if (changed()) {
            m_node.updateSetting("upnp", ToSetting(value, QVariant::Bool));
    ...
    case SpendZeroConfChange:
        if (changed()) {
            m_node.updateSetting("spendzeroconfchange", ToSetting(value, QVariant::Bool));
            setRestartRequired(true);
        }
        break;
#endif
    ...
    case Language:
        if (changed()) {
            m_node.updateSetting("lang", ToSetting(value, QVariant::String));
            setRestartRequired(true);
        }
        break;
    case CoinControlFeatures:
    ...
    case DatabaseCache:
        if (changed()) {
            m_node.updateSetting("dbcache", ToSetting(value, QVariant::Int));
            setRestartRequired(true);
        }
        break;
    case ThreadsScriptVerif:
        if (changed()) {
            m_node.updateSetting("par", ToSetting(value, QVariant::Int));
            setRestartRequired(true);
        }
        break;
    case Listen:
        if (changed()) {
            m_node.updateSetting("listen", ToSetting(value, QVariant::Bool));
            setRestartRequired(true);
        }
        break;
    default:
...
void OptionsModel::checkAndMigrate()
{
    ...
    auto migrate_setting = [&](OptionID option, const QString& qt_name, const std::string& name) {
        ...
        if (m_node.getPersistentSetting(name).isNull()) {
            if (option == ProxyIP) {
