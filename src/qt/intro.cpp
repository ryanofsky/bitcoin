...
{
    int64_t prune_target_mib = PruneSetting::Get(gArgs).value_or(0);
    ...
    m_prune_target_gb{GetPruneTargetGB()}
{
    ...
    if (PruneSetting::Get(gArgs)) {
        m_prune_checkbox_is_default = false;
        ui->prune->setChecked(PruneSetting::Get(gArgs).value_or(0) >= 1);
...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
    ...
    if(!DatadirSetting::Get(gArgs).value_or("").empty())
        return true;
