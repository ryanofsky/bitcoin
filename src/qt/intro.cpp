...
||||||| parent of 5855150efae7 (Make GUI and CLI tools use the same datadir)
    did_show_intro = false;
...
>>>>>>> 5855150efae7 (Make GUI and CLI tools use the same datadir)
            return false;
        }
    }
    if (link_datadir) {
...
bool Intro::showIfNeeded(bool& did_show_intro, fs::path& datadir, int64_t& prune_MiB)
{
    ...
    if (legacy_datadir_str.isValid()) {
        ...
        if (legacy_datadir.empty() || legacy_datadir == datadir || legacy_datadir == default_datadir) {
            ...
            messagebox.setWindowTitle(PACKAGE_NAME);
