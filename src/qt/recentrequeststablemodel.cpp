...
bool RecentRequestsTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    if(count > 0 && row >= 0 && (row+count) <= list.size())
    {
        for (int i = 0; i < count; ++i)
        {
            ...
            if (!walletModel->wallet().setAddressReceiveRequest(DecodeDestination(rec->recipient.address.toStdString()), util::ToString(rec->id), ""))
                return false;
        }
...
void RecentRequestsTableModel::addNewRequest(const SendCoinsRecipient &recipient)
{
    ...
    if (!walletModel->wallet().setAddressReceiveRequest(DecodeDestination(recipient.address.toStdString()), util::ToString(newEntry.id), ss.str()))
        return;
