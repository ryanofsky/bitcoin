...
#ifndef BITCOIN_CLI_SETTINGS_H
#define BITCOIN_CLI_SETTINGS_H
...
using ConfSetting = common::Setting<
    "-conf=<file>", std::optional<char>, {.legacy = true},
...
using GetinfoSetting = common::Setting<
    "-getinfo", std::optional<bool>, {.legacy = true},
...
using ColorSetting = common::Setting<
    "-color=<when>", std::optional<std::string>, {.legacy = true, .disallow_negation = true},
    "Color setting for CLI output (default: %s). Valid values: always, auto (add color codes when standard output is connected to a terminal and OS is not WIN32), never.">
    ::HelpFn<[](const auto& fmt) { return strprintf(fmt, DEFAULT_COLOR_SETTING); }>;

using NamedSetting = common::Setting<
    "-named", bool, {.legacy = true},
    "Pass named instead of positional arguments (default: %s)">
    ::Default<DEFAULT_NAMED>;

using RpcclienttimeoutSetting = common::Setting<
    "-rpcclienttimeout=<n>", int64_t, {.legacy = true},
...
using RpcconnectSetting = common::Setting<
    "-rpcconnect=<ip>", std::string, {.legacy = true},
...
using RpcpasswordSetting = common::Setting<
    "-rpcpassword=<pw>", std::string, {.legacy = true},
...
using RpcuserSetting = common::Setting<
    "-rpcuser=<user>", std::string, {.legacy = true},
    "Username for JSON-RPC connections">;

using RpcwaitSetting = common::Setting<
    "-rpcwait", bool, {.legacy = true},
    "Wait for RPC server to start">;

using RpcwaittimeoutSetting = common::Setting<
    "-rpcwaittimeout=<n>", int64_t, {.legacy = true, .disallow_negation = true},
...
using RpcwalletSetting = common::Setting<
    "-rpcwallet=<walletname>", std::optional<std::string>, {.legacy = true},
...
using StdinwalletpassphraseSetting = common::Setting<
    "-stdinwalletpassphrase", bool, {.legacy = true},
