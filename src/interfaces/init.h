...
public:
    ...
    virtual std::unique_ptr<WalletClient> makeWalletClient(Chain& chain, std::vector<std::string> wallet_filenames) = 0;
};
...
public:
    ...
    std::unique_ptr<WalletClient> makeWalletClient(Chain& chain, std::vector<std::string> wallet_filenames) override;
