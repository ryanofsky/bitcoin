...
public:
    ...
    bool findAncestor(const uint256& block_hash, const uint256& ancestor_hash, int* height) override
    {
        LOCK(::cs_main);
        const CBlockIndex* block = LookupBlockIndex(block_hash);
        const CBlockIndex* ancestor = LookupBlockIndex(ancestor_hash);
        if (!block || !ancestor || block->GetAncestor(ancestor->nHeight) != block) return false;
