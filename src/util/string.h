...
struct ConstevalFormatString {
    ...
    constexpr static void Detail_CheckNumFormatSpecifiers(const char* str)
    {
        ...
        for (auto it{str}; *it != '\0'; ++it) {
            ...
            // Increment argument count and consume positional specifier, if present.
            add_arg();

            // Consume flags.
            while (*it == '#' || *it == '0' || *it == '-' || *it == ' ' || *it == '+') ++it;

            // Consume dynamic or static width.
            if (*it == '*') {
                ++it;
                add_arg();
            } else {
            ...
            // Consume dynamic or static precision.
            if (*it == '.') {
              ++it;
              if (*it == '*') {
                 ++it;
            ...
            // Do not allow '\0' to be used as final specifier character.
