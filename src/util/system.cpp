...
bool ArgsManager::InitSettings(std::string& error)
{
    if (EnableSettingsFile()) {
        ...
        if (!ReadSettingsFile(&errors)) {
            error = strprintf("Error reading settings file. Errors:\n- %s\n", Join(errors, "\n- "));
            return false;
        }
        if (!WriteSettingsFile(&errors)) {
            error = strprintf("Error writing settings file. Errors:\n- %s\n", Join(errors, "\n- "));
            return false;
        }
    }
    return true;
...
{
    fs::path path;
    if (!GetSettingsPath(&path, /* temp= */ false)) return true; // Do nothing if settings file disabled.
...
bool ArgsManager::WriteSettingsFile(std::vector<std::string>* errors) const
{
    ...
    if (!RenameOver(path_tmp, path)) {
        SaveErrors({strprintf("Error: Unable to rename settings file %s to %s\n", path_tmp.string(), path.string())}, errors);
        return false;
    }
    return true;
}
