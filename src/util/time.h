...
struct NodeClock : public std::chrono::system_clock {
    ...
    /** Same as row(), but callable by kernel code with a scarier name to flag
     * that it is nondetermenistic, and should only be used if it does not
     * affect validation results (e.g. for logging or flushing), or when kernel
     * applications have a way to bypass it and be deterministic. */
...
struct MockableSteadyClock : public std::chrono::steady_clock {
     ...
     *  Defined in time_nondet.cpp; not linked into bitcoinkernel. */
    static time_point now() noexcept;
    /** Same as row(), but callable by kernel code. See \ref
