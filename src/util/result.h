...
protected:
    ...
    //! Helper function to construct a new Result success value using the
    //! arguments provided.
    ...
    //! ConstructResult() overload peeling off a util::Error constructor
    //! argument and constructing a Result failure value using the arguments
    //! provided.
    template <typename Result, typename... Args>
    static void ConstructResult(Result& result, util::Error error, Args&&... args)
    {
        result.m_info.reset(new detail::ErrorInfo<F>{.failure{std::forward<Args>(args)...}, .error{std::move(error.message)}});
    }
...
public:
    ...
    Result(Args&&... args)
    {
        Base::ConstructResult(*this, std::forward<Args>(args)...);
    }
