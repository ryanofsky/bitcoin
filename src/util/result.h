...
//! Operator moving warning and error messages from a source result object to a
//! destination result object. Only moves message strings, does not change
//! success or failure values of either Result object.
//!
...
Result&& operator>>(Result&& src LIFETIMEBOUND, detail::ResultBase<T, F>& dest)
{
    dest.MoveMessages(src);
    return std::move(src);
}
