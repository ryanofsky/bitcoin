...
<<<<<<< HEAD
        ...
        // failure value below.
        src >> dst;
||||||| parent of 4e597b8dd938 (refactor: Add InfoType field to util::Result)
        // Use operator>> to move messages values first, then move success or
        // failure value below.
        std::move(src) >> dst;
=======
        ...
        std::move(src) >> dst;
>>>>>>> 4e597b8dd938 (refactor: Add InfoType field to util::Result)
        ...
        if (src) {
            ...
            } else if constexpr (!std::is_same_v<typename DstResult::FailureType, void>) {
...
=======
    if constexpr (!std::is_same_v<typename SrcResult::InfoType, void>) {
        ResultTraits<typename SrcResult::InfoType>::MergeInto(dst.GetInfo(), src.GetInfo());
    }
    if (src.GetMessages() && MessagesTraits<typename SrcResult::MessagesType>::HasMessages(*src.GetMessages())) {
        ResultTraits<typename SrcResult::MessagesType>::MergeInto(dst.EnsureMessages(), *src.GetMessages());
>>>>>>> 4e597b8dd938 (refactor: Add InfoType field to util::Result)
