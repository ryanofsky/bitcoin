...
||||||| parent of e2fa4802679 (move-only: move AddArg default values to headers)
static const char DEFAULT_RPCCONNECT[] = "127.0.0.1";
static const int DEFAULT_HTTP_CLIENT_TIMEOUT=900;
static constexpr int DEFAULT_WAIT_CLIENT_TIMEOUT = 0;
static const bool DEFAULT_NAMED=false;
=======
>>>>>>> e2fa4802679 (move-only: move AddArg default values to headers)
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::string& endpoint, const std::string& username)
{
    ...
    if (RpcPasswordSetting::Get(gArgs) == "") {
        ...
        rpc_credentials = username + ":" + RpcPasswordSetting::Get(gArgs);
    }
