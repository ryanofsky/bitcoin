...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    IpcconnectSetting::Register(argsman);
}
...
{
    auto ipcconnect{IpcconnectSetting::Get(gArgs, "auto")};
    if (ipcconnect == "0") return {}; // Do not attempt IPC if -ipcconnect is disabled.
    if (!RpcConnectSetting::Value(gArgs).isNull() && !RpcConnectSetting::Value(gArgs).isFalse()) {
...
||||||| parent of 5d2aa516a3a (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
...
>>>>>>> 5d2aa516a3a (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
...
=======
        strRPCUserColonPass = username + ":" + RpcPasswordSetting::Get(gArgs);
>>>>>>> 5d2aa516a3a (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
    }
...
static UniValue ConnectAndCallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    std::string username{RpcUserSetting::Get(gArgs)};
    do {
        try {
