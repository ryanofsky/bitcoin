...
static void ParseGetInfoResult(UniValue& result)
{
    ...
    if (ColorSetting::Get(gArgs)) {
        const std::string color{ColorSetting::Get(gArgs, DEFAULT_COLOR_SETTING)};
        if (color == "always") {
            should_colorize = true;
        } else if (color == "never") {
...
static UniValue GetNewAddress()
{
    ...
    if (RpcwalletSetting::Get(gArgs)) wallet_name = RpcwalletSetting::Get(gArgs, "");
    DefaultRequestHandler rh;
...
static int CommandLineRPC(int argc, char *argv[])
{
    ...
    try {
        ...
        if (nRet == 0) {
            ...
            if (error.isNull()) {
                if (GetinfoSetting::Get(gArgs, false)) {
