...
<<<<<<< HEAD
    if (start_height > stop_index->nHeight) {
        ...
        return false;
||||||| parent of 53a96c1ce706 (indexes, refactor: Remove remaining CBlockIndex* pointers from indexing code)
    if (start_height > stop_index->nHeight) {
...
>>>>>>> 53a96c1ce706 (indexes, refactor: Remove remaining CBlockIndex* pointers from indexing code)
    }
