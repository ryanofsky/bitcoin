...
CoinStatsIndex::CoinStatsIndex(BCLog::Logger& logger, std::unique_ptr<interfaces::Chain> chain, size_t n_cache_size, bool f_memory, bool f_wipe)
    : BaseIndex(logger, std::move(chain), "coinstatsindex")
{
    fs::path path{gArgs.GetDataDirNet() / "indexes" / "coinstats"};
    fs::create_directories(path);

    m_db = std::make_unique<CoinStatsIndex::DB>(logger, path / "db", n_cache_size, f_memory, f_wipe);
}
