...
||||||| parent of 9aee81f6c4b8 (indexes, refactor: Remove remaining CBlockIndex* uses in index Rewind methods)
    {
        LOCK(cs_main);
        const CBlockIndex* iter_tip{m_chainstate->m_blockman.LookupBlockIndex(current_tip.hash)};
        const CBlockIndex* new_tip_index{m_chainstate->m_blockman.LookupBlockIndex(new_tip.hash)};
...
=======
    if (!ReverseBlock(block)) {
        return false; // failure cause logged internally
>>>>>>> 9aee81f6c4b8 (indexes, refactor: Remove remaining CBlockIndex* uses in index Rewind methods)
    }
