...
||||||| parent of 831745330668 (indexes, refactor: Remove index RegisterValidationInterface call)
    LOCK(cs_main);
    CChain& index_chain = m_chainstate->m_chain;
...
||||||| parent of 8c3957de621e (indexes, refactor: Move more new block logic out of ThreadSync to blockConnected)
        std::chrono::steady_clock::time_point last_log_time{0s};
        std::chrono::steady_clock::time_point last_locator_write_time{0s};
=======
...
<<<<<<< HEAD
||||||| parent of 3214f6213ea0 (indexes, refactor: Add Commit CBlockLocator& argument)
    if (!Commit()) {
        // If commit fails, revert the best block index to avoid corruption.
...
>>>>>>> 3214f6213ea0 (indexes, refactor: Add Commit CBlockLocator& argument)
    return true;
}
...
||||||| parent of ea170e8c16ea (indexes: Avoid race, make -reindex-chainstate more efficient)
        //
        // The other case where the new block will connect to an ancestor of the
        // current block rather than the current block is when the m_synced flag
        // is set to true too early. For example if the index is synced to
