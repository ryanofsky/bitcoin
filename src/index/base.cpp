...
||||||| parent of 3cb5df955ee (indexes: Avoid race, make -reindex-chainstate more efficient)
...
>>>>>>> 3cb5df955ee (indexes: Avoid race, make -reindex-chainstate more efficient)
...
||||||| parent of db8f8386f30 (indexes, refactor: Add Commit CBlockLocator& argument)
            GetDB().WriteBestBlock(batch, GetLocator(*m_chain, m_best_block_index.load()->GetBlockHash()));
            ok = GetDB().WriteBatch(batch);
=======
            GetDB().WriteBestBlock(batch, locator);
            ok = GetDB().WriteBatch(batch);
>>>>>>> db8f8386f30 (indexes, refactor: Add Commit CBlockLocator& argument)
        }
    }
    if (!ok) {
...
void BaseIndex::BlockConnected(ChainstateRole role, const interfaces::BlockInfo& block_info)
=======
||||||| parent of 9b09fc1c284 (indexes, refactor: Move Rewind logic out of Rewind to blockDisconnected and Sync)
bool BaseIndex::Rewind(const CBlockIndex* current_tip, const CBlockIndex* new_tip)
{
    assert(current_tip->GetAncestor(new_tip->nHeight) == new_tip);
...
=======
>>>>>>> 9b09fc1c284 (indexes, refactor: Move Rewind logic out of Rewind to blockDisconnected and Sync)
