...
||||||| parent of e5f264113741 (indexes: Avoid race, make -reindex-chainstate more efficient)
        //
        // The other case where the new block will connect to an ancestor of the
        // current block rather than the current block is when the m_synced flag
        // is set to true too early. For example if the index is synced to
...
>>>>>>> e5f264113741 (indexes: Avoid race, make -reindex-chainstate more efficient)
    ...
    if (CustomAppend(block_info)) {
        ...
        FatalErrorf("%s: Failed to write block %s to index",
                   __func__, pindex->GetBlockHash().ToString());
        return;
||||||| parent of 8255f1ed9210 (indexes, refactor: Remove index validationinterface hooks)
    ...
    if (CustomAppend(block_info)) {
        ...
        return;
=======
>>>>>>> 8255f1ed9210 (indexes, refactor: Remove index validationinterface hooks)
    }
    return false;
}
...
<<<<<<< HEAD
        LogPrintf("%s: WARNING: Locator contains block (hash=%s) not on known best "
                  ...
                  __func__, locator_tip_hash.ToString(),
                  best_block_index->GetBlockHash().ToString());
||||||| parent of 4f356239f47a (indexes, refactor: Move Rewind logic out of Rewind to blockDisconnected and ThreadSync)
...
>>>>>>> 4f356239f47a (indexes, refactor: Move Rewind logic out of Rewind to blockDisconnected and ThreadSync)
        return true;
    }
    return false;
}

||||||| parent of 37d9b0332f9f (indexes: Rewrite chain sync logic, remove racy init)
bool BaseIndex::IgnoreBlockConnected(const interfaces::BlockInfo& block)
{
    // During initial sync, ignore validation interface notifications, only
    // process notifications from sync thread.
...
=======
>>>>>>> 37d9b0332f9f (indexes: Rewrite chain sync logic, remove racy init)
