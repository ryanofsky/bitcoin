...
||||||| parent of 459670c84fc (indexes, refactor: Remove index RegisterValidationInterface call)
...
BaseIndex::DB::DB(const fs::path& path, size_t n_cache_size, bool f_memory, bool f_wipe, bool f_obfuscate) :
>>>>>>> 459670c84fc (indexes, refactor: Remove index RegisterValidationInterface call)
    CDBWrapper{DBParams{
        .path = path,
...
=======
||||||| parent of ccebd8f6823 (indexes: Move sync thread from index to node)
void BaseIndex::Sync()
{
    auto notifications = WITH_LOCK(m_mutex, return m_notifications);
    // If m_notifications is null, it means Stop() was called before the Sync
...
=======
>>>>>>> ccebd8f6823 (indexes: Move sync thread from index to node)
