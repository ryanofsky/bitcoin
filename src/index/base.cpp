...
||||||| parent of 029e52325130 (indexes: Avoid race, make -reindex-chainstate more efficient)
            {
                LOCK(cs_main);
...
<<<<<<< HEAD
            ...
            if (last_locator_write_time + SYNC_LOCATOR_WRITE_INTERVAL < current_time) {
                SetBestBlockIndex(pindex->pprev);
                last_locator_write_time = current_time;
>>>>>>> 029e52325130 (indexes: Avoid race, make -reindex-chainstate more efficient)
                // No need to handle errors in Commit. See rationale above.
<<<<<<< HEAD
                Commit();
...
||||||| parent of 9b74dc0f9949 (indexes, refactor: Add Commit CBlockLocator& argument)
                Commit();
=======
                Commit(GetLocator(*m_chain, pindex->GetBlockHash()));
>>>>>>> 9b74dc0f9949 (indexes, refactor: Add Commit CBlockLocator& argument)
            }
            if (pindex_next->pprev != pindex && !Rewind(pindex, pindex_next->pprev)) {
...
||||||| parent of 1b9da22a98dd (indexes, refactor: Move more new block logic out of ThreadSync to blockConnected)
            auto current_time{std::chrono::steady_clock::now()};
            if (last_log_time + SYNC_LOG_INTERVAL < current_time) {
                LogPrintf("Syncing %s with block chain from height %d\n",
                          GetName(), pindex->nHeight);
...
=======
>>>>>>> 1b9da22a98dd (indexes, refactor: Move more new block logic out of ThreadSync to blockConnected)
            CBlock block;
...
||||||| parent of e0b411f9f162 (indexes, refactor: Remove remaining CBlockIndex* uses in index CustomAppend methods)
            if (!CustomAppend(block_info)) {
                FatalErrorf("%s: Failed to write block %s to index database",
                           __func__, pindex->GetBlockHash().ToString());
                return;
...
>>>>>>> e0b411f9f162 (indexes, refactor: Remove remaining CBlockIndex* uses in index CustomAppend methods)
        }
    }
}
