...
||||||| parent of e2bc742917d (indexes, refactor: Remove remaining CBlockIndex* pointers from indexing code)
            CBlockIndex& block_index = *CHECK_NONFATAL(WITH_LOCK(::cs_main, return blockman.LookupBlockIndex(view->GetBestBlock())));
            return g_coin_stats_index->LookUpStats(block_index);
=======
            CBlockIndex& block_index = *CHECK_NONFATAL(WITH_LOCK(::cs_main, return blockman.LookupBlockIndex(view->GetBestBlock())));
            return g_coin_stats_index->LookUpStats({block_index.GetBlockHash(), block_index.nHeight});
>>>>>>> e2bc742917d (indexes, refactor: Remove remaining CBlockIndex* pointers from indexing code)
        }
    }
