    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = Assert(EnsureNodeContext(request.context).init)->ipc()) {
                ...
                // client pointer to a interfaces::Echo instance running in that
                // process. This is just for testing. A slightly more realistic
                // test of one process calling a difference process instead of
                // one process calling the same process would build a new
                // bitcoin-echo executable, and spawn bitcoin-echo here instead
                // of bitcoin-node. But using bitcoin-node avoids the need to
                // build and install a new binary not useful for anything except
