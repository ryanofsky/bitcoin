...
||||||| parent of a35a2a058872 (refactor: Add Chainstate m_validity and m_target_blockhash members)
    // Belt-and-suspenders check that we aren't attempting to advance the background
    // chainstate past the snapshot base block.
    if (WITH_LOCK(::cs_main, return m_disabled)) {
        LogPrintf("m_disabled is set - this chainstate should not be in operation. "
...
=======
    ...
    if (WITH_LOCK(::cs_main, return m_target_utxohash)) {
        LogPrintf("m_target_utxohash is set - this chainstate should not be in operation. "
            "Please report this as a bug. %s\n", PACKAGE_BUGREPORT);
>>>>>>> a35a2a058872 (refactor: Add Chainstate m_validity and m_target_blockhash members)
        return false;
    }
...
||||||| parent of e5bee04e6cfd (refactor: Delete ChainstateManager::SnapshotBlockhash() method)
    if (this->SnapshotBlockhash()) {
        return util::Error{Untranslated("Can't activate a snapshot-based chainstate more than once")};
    }

=======
>>>>>>> e5bee04e6cfd (refactor: Delete ChainstateManager::SnapshotBlockhash() method)
    {
        LOCK(::cs_main);
...
||||||| parent of f36304ed8aac (refactor: Add Chainstate::StoragePath() method)
bool ChainstateManager::ValidatedSnapshotCleanup()
=======
||||||| parent of c95d4917dada (refactor: Add ChainstateManager::CurrentChainstate() method)
const CBlockIndex* ChainstateManager::GetSnapshotBaseBlock() const
{
    return m_active_chainstate ? m_active_chainstate->SnapshotBase() : nullptr;
}
...
=======
>>>>>>> c95d4917dada (refactor: Add ChainstateManager::CurrentChainstate() method)
