...
||||||| parent of 9f89c918697 (refactor, validation: Return fatal errors from FlushStateToDisk)
...
=======
                if (!CheckDiskSpace(m_chainman.m_options.datadir, 48 * 2 * 2 * CoinsTip().GetCacheSize())) {
                    ...
                    result.update({util::Error{std::move(error)}, AbortFailure{.fatal = true}});
                    return result;
>>>>>>> 9f89c918697 (refactor, validation: Return fatal errors from FlushStateToDisk)
                }
...
||||||| parent of 5a10b1aaa47 (refactor, validation: Return fatal errors from assumeutxo snapshot functions)
bool ChainstateManager::ValidatedSnapshotCleanup(Chainstate& validated_cs, Chainstate& unvalidated_cs)
=======
...
>>>>>>> 5a10b1aaa47 (refactor, validation: Return fatal errors from assumeutxo snapshot functions)
{
    AssertLockHeld(::cs_main);
