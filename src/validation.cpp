...
||||||| parent of dceb837707b1 (refactor, validation: Return fatal errors from FlushStateToDisk)
            if (!CoinsTip().Flush())
                return FatalError(m_chainman.GetNotifications(), state, _("Failed to write to coin database."));
=======
            if (!CoinsTip().Flush()) {
...
>>>>>>> dceb837707b1 (refactor, validation: Return fatal errors from FlushStateToDisk)
            m_last_flush = nNow;
...
<<<<<<< HEAD
    if (m_blockman.m_blockfiles_indexed) {
        ...
        if (!ret) return false;
||||||| parent of 2c2134443ccf (refactor, blockstorage: Return fatal error from LoadBlockIndex)
...
>>>>>>> 2c2134443ccf (refactor, blockstorage: Return fatal error from LoadBlockIndex)
