...
bool ChainstateManager::LoadBlockIndex()
{
    ...
    if (!fReindex) {
        ...
        for (CBlockIndex* pindex : vSortedByHeight) {
            ...
            if (pindex->HaveNumChainTxs() || pindex->pprev == nullptr) {
...
void ChainstateManager::CheckBlockIndex()
{
    ...
    while (pindex != nullptr) {
        ...
        assert((pindexFirstNeverProcessed == nullptr || snap_block) == pindex->HaveNumChainTxs());
        assert((pindexFirstNotTransactionsValid == nullptr || snap_block) == pindex->HaveNumChainTxs());
        ...
        // Expect nChainTx not be set if nTx is unset (happens when the block
        // hasn't been processed yet), or if there is a previous block and the
        // previous nChainTx is unset (happens when one of its predecessors
        // hasn't been processed yet).
        bool unset_chain_tx = pindex->nTx == 0 || (pindex->pprev && pindex->pprev->nChainTx);
        assert((pindex->nChainTx == pindex->nTx + prev_chain_tx)
               || (pindex->nChainTx == 0 && unset_chain_tx)
               // Assumeutxo snapshot will have a nChainTx value taken from
               // snapshot metadata, even if previous nChainTx is 0.
               || (pindex->nChainTx != 0 && unset_chain_tx && snap_block));
        // Chainstate-specific checks on setBlockIndexCandidates
        for (auto c : GetAll()) {
            const bool is_active = c == &ActiveChainstate();
            if (c->m_chain.Tip() == nullptr) continue;
            if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) && (pindexFirstNeverProcessed == nullptr || (is_active && snap_block))) {
                if (pindexFirstInvalid == nullptr) {
                    ...
                    //
                    if (pindexFirstMissing == nullptr || pindex == c->m_chain.Tip() || (is_active && snap_block)) {
        ...
        // When reaching an assumeutxo snapshot block, reset pointers to earlier
        // blocks that have not been downloaded or validated yet. The checks in
        // this function can be stricter if they pretend the snapshot block and
        // blocks before it have been validated and downloaded even though this
