...
||||||| parent of 08200e60968 (refactor, validation: Return fatal errors from block connect functions)
        CCoinsViewCache view(&CoinsTip());
        bool rv = ConnectBlock(*block_to_connect, state, pindexNew, view);
=======
        CCoinsViewCache view(&CoinsTip());
        bool rv = bool{ConnectBlock(*block_to_connect, state, pindexNew, view) >> result};
>>>>>>> 08200e60968 (refactor, validation: Return fatal errors from block connect functions)
        if (m_chainman.m_options.signals) {
