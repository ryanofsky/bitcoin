...
>>>>>>> 1acd64b64334 (refactor, blockstorage: Return fatal errors from block writes)
        return false;
||||||| parent of 6d9defff6b76 (refactor, validation: Return fatal errors from block connect functions)
        return false;
=======
        return result;
>>>>>>> 6d9defff6b76 (refactor, validation: Return fatal errors from block connect functions)
    }
...
||||||| parent of 0244a91fbad6 (refactor, validation: Return fatal errors from activate best chain functions)
                if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace)) {
=======
...
>>>>>>> 0244a91fbad6 (refactor, validation: Return fatal errors from activate best chain functions)
