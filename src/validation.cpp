...
void ChainstateManager::CheckBlockIndex()
{
    ...
    while (pindex != nullptr) {
        ...
        for (auto c : GetAll()) {
            ...
            //   potential candidate.
            //
            // - If pindex or one of its parent blocks back the genesis block or
            //   an assumeutxo snapshot never downloaded transactions
            //   (pindexFirstNeverProcessed is non-null), it should not be in
            //   a candidate, and this will be asserted below. The
            //   only exception is if pindex itself is an assumeutxo snapshot
            //   block. Then it is also a potential candidate.
            if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) && (pindexFirstNeverProcessed == nullptr || pindex == snap_base)) {
                ...
                if (pindexFirstInvalid == nullptr) {
                    ...
                    // an assumeutxo snapshot block downloaded transactions, and
                    // the transactions were not pruned (pindexFirstMissing is
                    // null), it is a potential candidate.
                    //
                    // Or if the chain tip is already pointing at pindex, then
                    // pindex is a potential candidate.
                    //
