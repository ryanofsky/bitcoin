...
bool Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    do {
        ...
        if (WITH_LOCK(::cs_main, return m_target_block && m_chain.Tip() == m_target_block)) {
...
void Chainstate::TryAddBlockIndexCandidate(CBlockIndex* pindex)
{
    ...
    if (!m_target_block) {
        ...
        // never make progress).
        if (m_target_block->GetAncestor(pindex->nHeight) == pindex) {
            setBlockIndexCandidates.insert(pindex);
        }
    }
}
            ...
            if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) && pindexFirstNeverProcessed == nullptr) {
                if (pindexFirstInvalid == nullptr) {
                    ...
                    if ((pindexFirstMissing == nullptr || pindex == c->m_chain.Tip())) {
                        ...
                        if (!c->m_target_block || c->m_target_block->GetAncestor(pindex->nHeight) == pindex) {
        ...
        bool in_memory)
{
    ...
    AddChainstate(std::move(snapshot_chainstate));
...
Chainstate* ChainstateManager::DetectSnapshotChainstate()
{
    ...
    return &this->AddChainstate(std::make_unique<Chainstate>(nullptr, m_blockman, *this, *base_blockhash));
}
...
Chainstate& ChainstateManager::AddChainstate(std::unique_ptr<Chainstate> chainstate)
{
    ...
    Chainstate& active = MostWorkChainstate();
    assert(active.m_chain.Tip()->nChainWork < chainstate->m_chain.Tip()->nChainWork);
...
std::string Chainstate::ToString() const
{
...
std::optional<std::pair<const CBlockIndex*, const CBlockIndex*>> ChainstateManager::GetHistoricalBlockRange() const
{
    ...
    return std::make_pair(chainstate->m_chain.Tip(), chainstate->m_target_block);
}
