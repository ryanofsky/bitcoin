...
void Chainstate::TryAddBlockIndexCandidate(CBlockIndex* pindex)
{
    ...
    // or more work than our current tip. The tip will be null on startup
    // because Chainstate::LoadChainTip is called after
    // ChainstateManager::LoadBlockIndex, so add every block in this case.
    if (m_chain.Tip() != nullptr && setBlockIndexCandidates.value_comp()(pindex, m_chain.Tip())) {
        return;
    }

    // Always add the current chain tip as a tip candidate, because of code
    // written with this assumption. A special case for this is neccesary
    // because in the case of newly created assumeutxo snapshot chainstate, the
    // snapshot block will not be validated or have transaction data.
