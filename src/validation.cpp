...
void ChainstateManager::ReceivedBlockTransactions(const CBlock& block, CBlockIndex* pindexNew, const FlatFilePos& pos)
{
    ...
    if (!Assume(pindexNew->nChainTx == 0 ||
        ...
        LogWarning("Internal bug detected: block %d has unexpected nChainTx %i (%s %s). Please report this issue here: %s\n",
            pindexNew->nHeight, pindexNew->nChainTx, PACKAGE_NAME, FormatFullVersion(), PACKAGE_BUGREPORT);
    }
    ...
    if (pindexNew->pprev == nullptr || pindexNew->pprev->HaveNumChainTxs()) {
        ...
        while (!queue.empty()) {
            ...
            if (!Assume(pindex->nChainTx == 0 || pindex->nChainTx == chain_tx)) {
                LogWarning("Internal bug detected: block %d has unexpected nChainTx %i that should be %i (%s %s). Please report this issue here: %s\n",
                   pindex->nHeight, pindex->nChainTx, chain_tx, PACKAGE_NAME, FormatFullVersion(), PACKAGE_BUGREPORT);
            }
        ...
        for (auto c : GetAll()) {
            ...
            if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) && (pindexFirstNeverProcessed == nullptr || pindex == snap_base)) {
                ...
                if (pindexFirstInvalid == nullptr) {
                    ...
                    // is null), it is a potential candidate. If any blocks were
                    // pruned between pindex the current chain tip, pindex may
                    // get added temporarily to setBlockIndexCandidates, but
                    // will get be moved to m_blocks_unlinked.
                    //
                    ...
                    // potential candidate.
                    //
                    // This check could potentially be more strict to verify if
                    // that if all blocks between the chain tip and pindex have
                    // data, pindex must be a candidate.
