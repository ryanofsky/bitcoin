    ...
    bool fAddToMempool)
{
    ...
    const auto filter_final_and_mature = [&](CTxMemPool::txiter it)
        ...
        if (TestLockPointValidity(m_chain, lp)) {
            ...
            const std::optional<LockPoints> new_lock_points{CalculateLockPointsAtTip(*Assert(m_log.logger), m_chain.Tip(), view_mempool, tx)};
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    const std::optional<LockPoints> lock_points{CalculateLockPointsAtTip(*Assert(m_pool.m_log.logger), m_active_chainstate.m_chain.Tip(), m_view, tx)};
                                     ...
                                     PackageValidationState& package_state)
{
    ...
    const util::log::Source log_packages{BCLog::TXPACKAGES, m_pool.m_log.logger};
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    const util::log::Source log_bench{BCLog::BENCH, m_log.logger};
    ...
    int nManualPruneHeight)
{
    ...
    const util::log::Source log_prune{BCLog::PRUNE, m_log.logger};
...
<<<<<<< HEAD
||||||| parent of c864c8a84fd (refactor: Log kernel output to local log instances)
...
>>>>>>> c864c8a84fd (refactor: Log kernel output to local log instances)
    ...
    const std::string& warning_messages) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    LogPrintLevel_(chainman.m_log, util::log::Level::Info, /*should_ratelimit=*/false, "%s%s: new best=%s height=%d version=0x%08x log2_work=%f tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)%s\n",
                   prefix, func_name,
...
{
    const util::log::Source& log{active_chainstate.m_log};
    BlockValidationState state;
...
{
    AssertLockHeld(cs_main);
    const util::log::Source& log{chainstate.m_log};
    ...
    std::multimap<uint256, FlatFilePos>* blocks_with_unknown_parent)
{
    ...
    const util::log::Source log_reindex{BCLog::REINDEX, m_log.logger};
...
[[nodiscard]] static bool DeleteCoinsDBFromDisk(const util::log::Source& log, const fs::path db_path, bool is_snapshot)
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    AssertLockHeld(::cs_main);
