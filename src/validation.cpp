...
||||||| parent of 39194c653437 (refactor, validation: Switch to result.Update)
    auto cleanup_bad_snapshot = [&](bilingual_str&& reason) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
=======
    auto cleanup_bad_snapshot = [&](bilingual_str&& reason) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        result.Update(util::Error{std::move(reason)});
<<<<<<< HEAD
>>>>>>> 39194c653437 (refactor, validation: Switch to result.Update)
        this->MaybeRebalanceCaches();
||||||| parent of 8590e7b282bf (refactor, validation: Return fatal errors from FlushStateToDisk)
        this->MaybeRebalanceCaches();
=======
        // Ignore failure value, do not treat flush error as failure.
        this->MaybeRebalanceCaches() >> result;
>>>>>>> 8590e7b282bf (refactor, validation: Return fatal errors from FlushStateToDisk)
