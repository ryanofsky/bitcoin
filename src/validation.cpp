...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    m_chainman.MaybeValidateSnapshot(*this, current_chainstate);
...
SnapshotCompletionResult ChainstateManager::MaybeValidateSnapshot(Chainstate& validated_cs, Chainstate& assumed_valid_cs)
{
    AssertLockHeld(cs_main);

    // If the snapshot does not need to be validated...
    if (assumed_valid_cs.Validity() != ChainValidity::ASSUMED_VALID ||
            // Or if either chainstate is unusable...
            !assumed_valid_cs.m_from_snapshot_blockhash ||
            validated_cs.Validity() != ChainValidity::VALIDATED ||
            !validated_cs.m_chain.Tip() ||
            // Or the validated chainstate is not targeting the snapshot block...
            !validated_cs.m_target_blockhash ||
            *validated_cs.m_target_blockhash != *assumed_valid_cs.m_from_snapshot_blockhash ||
    ...
    auto handle_invalid_snapshot = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        bilingual_str user_error = strprintf(_(
            ...
            CLIENT_NAME, assumed_valid_cs.m_chain.Height(),
        ...
        assumed_valid_cs.m_validity = ChainValidity::INVALID;

        auto rename_result = assumed_valid_cs.InvalidateCoinsDBOnDisk();
        if (!rename_result) {
    ...
    LogPrintf("[snapshot] snapshot beginning at %s has been fully validated\n",
        assumed_valid_cs.m_from_snapshot_blockhash->ToString());

    assumed_valid_cs.m_validity = ChainValidity::VALIDATED;
...
Chainstate* ChainstateManager::DetectAssumedValidChainstate()
{
...
bool ChainstateManager::ValidatedSnapshotCleanup(Chainstate& validated_cs, Chainstate& assumed_valid_cs)
{
    ...
    const fs::path assumed_valid_path{assumed_valid_cs.StoragePath()};
