...
||||||| parent of 1d477c711599 (refactor: Convert ChainstateRole enum to struct)
...
>>>>>>> 1d477c711599 (refactor: Convert ChainstateRole enum to struct)
        return;
    }
...
||||||| parent of 811bb3db84a4 (refactor: Add Chainstate m_assumeutxo and m_target_utxohash members)
    // Belt-and-suspenders check that we aren't attempting to advance the background
    // chainstate past the snapshot base block.
    if (WITH_LOCK(::cs_main, return m_disabled)) {
...
=======
    ...
    if (WITH_LOCK(::cs_main, return m_target_utxohash)) {
        LogPrintf("m_target_utxohash is set - this chainstate should not be in operation. "
            "Please report this as a bug. %s\n", CLIENT_BUGREPORT);
>>>>>>> 811bb3db84a4 (refactor: Add Chainstate m_assumeutxo and m_target_utxohash members)
        return false;
    }
...
||||||| parent of 21d638c92f56 (refactor: Pass chainstate parameters to MaybeCompleteSnapshotValidation)
    assert(index_new.GetBlockHash() == snapshot_blockhash);
    assert(index_new.nHeight == snapshot_base_height);
=======
    CCoinsViewDB& validated_coins_db = validated_cs.CoinsDB();
    validated_cs.ForceFlushStateToDisk();
>>>>>>> 21d638c92f56 (refactor: Pass chainstate parameters to MaybeCompleteSnapshotValidation)
...
||||||| parent of a08822ddcf3e (refactor: Add Chainstate::StoragePath() method)
        auto src_str = fs::PathToString(snapshot_datadir);
        auto dest_str = fs::PathToString(invalid_path);
...
>>>>>>> a08822ddcf3e (refactor: Add Chainstate::StoragePath() method)
...
<<<<<<< HEAD
    ...
    if (!DeleteCoinsDBFromDisk(snapshot_datadir, /*is_snapshot=*/ true)) {
        LogError("Deletion of %s failed. Please remove it manually to continue reindexing.",
                  fs::PathToString(snapshot_datadir));
||||||| parent of f85ff75dad4c (refactor: Add ChainstateManager::m_chainstates member)
    Assert(m_snapshot_chainstate);
    Assert(m_ibd_chainstate);
...
>>>>>>> f85ff75dad4c (refactor: Add ChainstateManager::m_chainstates member)
        return false;
    }
