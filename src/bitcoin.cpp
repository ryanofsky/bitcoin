...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    auto try_exec = [&](fs::path exe_path, bool allow_notfound = true) {
        ...
        if (util::ExecVp(exec_args[0], (char*const*)exec_args.data()) == -1) {
            ...
            // Use generic_category because execvp/_spawnvp set POSIX errno
            // values (ENOENT, EINVAL, etc.), not Win32 error codes. On Windows,
            // system_category maps Win32 codes so the same integer produces a
            // wrong message (e.g. EINVAL=22 → Win32 "The device does not
            // recognize the command" instead of "Invalid argument").
