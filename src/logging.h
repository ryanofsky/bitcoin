...
//! Internal helper. Get log source object from macro argument, if argument is
//! BCLog::Source object or a category constant that can be BCLog::Source can be
//! constructed from.
static inline const BCLog::Source& _LogSource(const BCLog::Source& source LIFETIMEBOUND) { return source; }

//! Internal helper. Get log source object from macro argument, if argument is a
//! custom log source with a log_source member.
template <typename Source>
...
static inline const BCLog::Source _LogSource(std::string_view fmt) { return {}; }
...
//! Internal helper. Return first args of a __VA_ARGS__ pack.
#define _FirstArg(arg, ...) arg

//! Internal helper. Check level and log. Avoid evaluating arguments if not logging.
