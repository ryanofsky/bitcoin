...
namespace BCLog {
    ...
    //! Log messages at Info level and above are always emitted regardless of category settings.
    constexpr size_t NUM_CONDITIONAL_LEVELS{static_cast<size_t>(Level::Info)};
    ...
    private:
        ...
        //! This array only has entries for conditional log levels (Trace, Debug) since
        //! unconditional log messages (Trace, Warning, Error) can't be turned off.
        std::array<std::atomic<CategoryMask>, NUM_CONDITIONAL_LEVELS> m_levels{BCLog::NONE, BCLog::NONE};
    ...
    public:
        ...
        using LogLevels = std::array<CategoryMask, NUM_CONDITIONAL_LEVELS>;
