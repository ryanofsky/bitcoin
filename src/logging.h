...
//! Log macros that avoid evaluating their arguments if the log category is not
//! enabled at the specified level. The macros all accept a BCLog::Source
//! parameter followed by a printf-style format string and arguments.
//
//! LogError(), LogWarning(), and LogInfo() are all enabled by default, so they
//! should be called infrequently, in cases where they will not spam the log and
//! take up disk space.
//!
...
//! should be used for fine-grained tracing that will be helpful to developers.
//!
//! For more information, see the -debug and -loglevel documentation, or the
//! "Logging" section of developer notes.
