...
uint16_t GetListenPort()
{
    ...
    for (const std::string& bind_arg : BindSetting::Get(gArgs).value_or(std::vector<std::string>{})) {
    ...
    for (const std::string& whitebind_arg : WhitebindSetting::Get(gArgs).value_or(std::vector<std::string>{})) {
    ...
    return static_cast<uint16_t>(PortSetting::Get(gArgs).value_or(Params().GetDefaultPort()));
}
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect, Span<const std::string> seed_nodes)
{
    ...
    const bool dnsseed = DnsseedSetting::Get(gArgs).value_or(DEFAULT_DNSSEED);
    bool add_fixed_seeds = FixedseedsSetting::Get(gArgs);
    const bool use_seednodes{SeednodeSetting::Get(gArgs)};
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (!DnsseedSetting::Get(gArgs).value_or(DEFAULT_DNSSEED))
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    if (CapturemessagesSetting::Get(gArgs).value_or(false)) {
