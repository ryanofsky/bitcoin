...
||||||| parent of bc5e4996596 (common: Disallow calling IsArgSet() on ALLOW_LIST options)
    BOOST_CHECK(testArgs.IsArgSet("-a") && testArgs.IsArgSet("-b") && testArgs.IsArgSet("-ccc")
                && !testArgs.IsArgSet("f") && !testArgs.IsArgSet("-d"));
=======
    BOOST_CHECK(testArgs.IsArgSet("-a") && testArgs.IsArgSet("-b") && testArgs.TestArgSet("-ccc")
                && !testArgs.IsArgSet("f") && !testArgs.IsArgSet("-d"));
>>>>>>> bc5e4996596 (common: Disallow calling IsArgSet() on ALLOW_LIST options)
    testArgs.LockSettings([&](const common::Settings& s) {
...
BOOST_AUTO_TEST_CASE(util_GetBoolArgEdgeCases)
{
    ...
    BOOST_REQUIRE(testArgs.TestArgList("-bar").size() == 1);
    BOOST_CHECK(testArgs.TestArgList("-bar").front() == "");
}
...
BOOST_AUTO_TEST_CASE(util_ReadConfigStream)
{
    ...
    BOOST_REQUIRE(test_args.TestArgList("-a").size() == 1);
    BOOST_CHECK(test_args.TestArgList("-a").front() == "");
    BOOST_REQUIRE(test_args.TestArgList("-b").size() == 1);
    BOOST_CHECK(test_args.TestArgList("-b").front() == "1");
    ...
    BOOST_REQUIRE(test_args.TestArgList("-ggg").size() == 1);
    BOOST_CHECK(test_args.TestArgList("-ggg").front() == "1");
    ...
    BOOST_CHECK(test_args.GetArg("-d", "xxx") == "e");
    BOOST_CHECK(test_args.TestArgList("-ccc").size() == 2);
    BOOST_CHECK(test_args.TestArgString("-h", "xxx") == "0");
    ...
    BOOST_CHECK(test_args.TestArgList("-ccc").size() == 1);
    BOOST_CHECK(test_args.TestArgString("-h", "xxx") == "0");
}
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions, bool soft_set, bool force_set,
        ...
        unsigned int flags = ArgsManager::ALLOW_ANY | ArgsManager::ALLOW_LIST;
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions) {
        ...
        parser.AddArg("-testnet4", "testnet4", ArgsManager::ALLOW_ANY | ArgsManager::ALLOW_LIST, OptionsCategory::OPTIONS);
