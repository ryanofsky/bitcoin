...
void IpcSocketPairTest()
{
    ...
    std::array<mp::Stream, 2> fds;
    std::promise<void> promise;
    std::thread thread([&]() {
        protocol->serve(*init, [&] {
            fds = protocol->makeSocketPair();
            promise.set_value();
            return std::move(fds[0]);
        });
    });
    promise.get_future().wait();
    std::unique_ptr<interfaces::Init> remote_init{protocol->connect(std::move(fds[1]))};
