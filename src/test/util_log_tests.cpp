...
BOOST_AUTO_TEST_CASE(dispatcher_callback_receives_entry)
{
    ...
    dispatcher.Log({.category = test_category, .level = Level::Info, .should_ratelimit = true, .source_loc = SourceLocation{__func__}, .message = "test message 123"});
...
BOOST_AUTO_TEST_CASE(dispatcher_multiple_callbacks)
{
    ...
    dispatcher.Log({.category = 0, .level = Level::Info, .should_ratelimit = false, .source_loc = SourceLocation{__func__}, .message = "test"});
...
BOOST_AUTO_TEST_CASE(dispatcher_skips_when_not_enabled)
{
    ...
    dispatcher.Log({.category = 0, .level = Level::Info, .should_ratelimit = false, .source_loc = SourceLocation{__func__}, .message = "this should not crash"});
}
