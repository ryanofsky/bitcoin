...
BOOST_FIXTURE_TEST_CASE(chainstatemanager, TestChain100Setup)
{
    ...
    BOOST_CHECK(LL(!manager.MostWorkChainstate().SnapshotBase()));
    ...
    BOOST_CHECK_EQUAL(LL(manager.MostWorkChainstate().SnapshotBase()->GetBlockHash()), snapshot_blockhash);
    BOOST_CHECK(LL(manager.MostWorkChainstate().SnapshotBase()));
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_rebalance_caches, TestChain100Setup)
{
    ...
    // Create a snapshot-based chainstate.
    //
    CBlockIndex* snapshot_base{LL(manager.ActiveChain()[manager.ActiveChain().Height() / 2])};
    Chainstate& c2{LL(manager.AddSnapshotChainstate(std::make_unique<Chainstate>(nullptr, manager.m_blockman, manager, *snapshot_base->phashBlock)))};
    chainstates.push_back(&c2);
    c2.InitCoinsDB(
        /*cache_size_bytes=*/1 << 23, /*in_memory=*/true, /*should_wipe=*/false);
...
struct SnapshotTestSetup : TestChain100Setup {
    ...
    std::tuple<Chainstate*, Chainstate*> SetupSnapshot()
    {
        ...
        BOOST_CHECK(LL(!chainman.MostWorkChainstate().SnapshotBase()));
        ...
        BOOST_CHECK_EQUAL(
            *chainman.ActiveChainstate().m_from_snapshot_blockhash,
            LL(chainman.MostWorkChainstate().SnapshotBase()->GetBlockHash()));
...

        {
            ...
            BOOST_CHECK_EQUAL(
                *node::ReadSnapshotBaseBlockhash(found),
                chainman.MostWorkChainstate().SnapshotBase()->GetBlockHash());
        ...
        uint256 loaded_snapshot_blockhash{LL(chainman.MostWorkChainstate().SnapshotBase()->GetBlockHash())};
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_loadblockindex, TestChain100Setup)
{
    ...
    Chainstate& cs2 = LL(chainman.AddSnapshotChainstate(std::make_unique<Chainstate>(nullptr, chainman.m_blockman, chainman, *assumed_base->phashBlock)));
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_completion, SnapshotTestSetup)
{
    ...
    res = LL(chainman.MaybeCompleteSnapshotValidation(*Assert(chainman.HistoricalChainstate()), active_cs));
    BOOST_CHECK_EQUAL(res, SnapshotCompletionResult::SUCCESS);

    BOOST_CHECK(LL(chainman.MostWorkChainstate().Validity() == ChainValidity::VALIDATED));
    BOOST_CHECK(LL(chainman.MostWorkChainstate().SnapshotBase()));
    BOOST_CHECK_EQUAL(LL(chainman.HistoricalChainstate()), nullptr);
    ...
    res = LL(chainman.MaybeCompleteSnapshotValidation(active_cs, active_cs));
