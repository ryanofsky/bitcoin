    ...
    : m_args{}
{
    ...
    if (!TestdatadirSetting::Get(*m_node.args)) {
        ...
        fs::path root_dir{TestdatadirSetting::Get(*m_node.args).value_or(fs::path{})};
...
void ChainTestingSetup::LoadVerifyActivateChainstate()
{
    ...
    options.check_blocks = CheckblocksSetting::Get(m_args).value_or(DEFAULT_CHECKBLOCKS);
    options.check_level = ChecklevelSetting::Get(m_args).value_or(DEFAULT_CHECKLEVEL);
    options.require_full_verification = CheckblocksSetting::Get(m_args) || ChecklevelSetting::Get(m_args);
    ...
    : ChainTestingSetup(chainType, opts)
{
    ...
    m_node.addrman = std::make_unique<AddrMan>(*m_node.netgroupman,
                                               /*deterministic=*/false,
                                               CheckaddrmanSetting::Get(*m_node.args).value_or(0));
