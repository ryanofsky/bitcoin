...
||||||| parent of c42d27d8c844 (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
    if (!m_node.args->IsArgSet("-testdatadir")) {
...
=======
    if (TestdatadirSetting::Value(*m_node.args).isNull()) {
        const auto now{TicksSinceEpoch<std::chrono::nanoseconds>(SystemClock::now())};
        m_path_root = fs::temp_directory_path() / TEST_DIR_PATH_ELEMENT / test_name / util::ToString(now);
>>>>>>> c42d27d8c844 (scripted-diff: Replace AddArgs / GetArgs calls with Setting Register / Get calls)
