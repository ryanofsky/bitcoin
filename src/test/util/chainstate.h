    ...
    if (reset_chainstate) {
        {
            ...
            while (pindex && pindex != chain.m_chain.Tip()) {
                ...
                // BLOCK_VALID_TREE. To make test setup more realistic and
                // satisfy consistency checks in CheckBlockIndex(), also reset
                // transaction counts and block sequence id that are set when
                // blocks are received.
