...
static void MultiAddTimeData(BCLog::Logger& logger, int n, int64_t offset)
{
    static int cnt = 0;
    for (int i = 0; i < n; ++i) {
        CNetAddr addr;
        addr.SetInternal(ToString(++cnt));
        AddTimeData(logger, addr, offset);
    }
}
...
BOOST_AUTO_TEST_CASE(addtimedata)
{
    ...
    MultiAddTimeData(m_logger, 3, DEFAULT_MAX_TIME_ADJUSTMENT + 1);
    // Filter size is 1 + 3 = 4: It is always initialized with a single element (offset 0)

    {
        ASSERT_DEBUG_LOG(m_logger, "Please check that your computer's date and time are correct!");
        MultiAddTimeData(m_logger, 1, DEFAULT_MAX_TIME_ADJUSTMENT + 1); //filter size 5
    }
    ...
    MultiAddTimeData(m_logger, 4, 100); // filter size 9
    BOOST_CHECK_EQUAL(GetTimeOffset(), 100);
    MultiAddTimeData(m_logger, 10, -100); //filter size 19
    BOOST_CHECK_EQUAL(GetTimeOffset(), -100);
    ...
    MultiAddTimeData(m_logger, nfill, 100);
    MultiAddTimeData(m_logger, nfill, -100); //filter size MAX_SAMPLES - 3
    BOOST_CHECK_EQUAL(GetTimeOffset(), -100);

    MultiAddTimeData(m_logger, 2, 100);
    ...
    MultiAddTimeData(m_logger, 2, 100); // filter median is 100 now, but nTimeOffset will not change
