...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection)
{
    ...
    auto peerman = PeerManager::make(*connman, *m_node.addrman, nullptr, *m_node.chainman, *m_node.mempool, m_logger, {});
    NodeId id{0};
    std::vector<CNode*> nodes;

    // Connect a localhost peer.
    {
        ASSERT_DEBUG_LOG(m_logger, "Added connection to 127.0.0.1:8333 peer=1");
    ...
    for (int i = 0; i < 10; ++i) {
        ASSERT_DEBUG_LOG(m_logger, "Not opening a connection to localhost, already connected to 127.0.0.1:8333");
