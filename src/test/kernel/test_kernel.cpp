...
BOOST_AUTO_TEST_CASE(btck_context_set_mock_time_tests)
{
    ...
    BOOST_CHECK_EXCEPTION(context.SetMockTime(std::chrono::seconds{-1}), std::runtime_error, HasReason{"timestamp out of range"});
    BOOST_CHECK_EXCEPTION(context.SetMockTime(max_time + std::chrono::seconds{1}), std::runtime_error, HasReason{"timestamp out of range"});
    ...
    context.SetMockTime(block_time - std::chrono::hours{3});
    ...
    context.SetMockTime(std::nullopt);
}
