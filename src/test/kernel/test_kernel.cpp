...
BOOST_AUTO_TEST_CASE(btck_clock_tests)
{
    auto test_directory{TestDirectory{"clock_test_bitcoin_kernel"}};
    auto notifications{std::make_shared<TestKernelNotifications>()};
    auto context{create_context(notifications, ChainType::REGTEST)};
    auto chainman{create_chainman(
        test_directory, /*reindex=*/false, /*wipe_chainstate=*/false,
    ...
    BOOST_CHECK_EXCEPTION(clock.SetTime(std::chrono::seconds{-1}), std::runtime_error, HasReason("timestamp out of range"));
    BOOST_CHECK_EXCEPTION(clock.SetTime(max_time + std::chrono::seconds{1}), std::runtime_error, HasReason("timestamp out of range"));

    Block block{hex_string_to_byte_vec(REGTEST_BLOCK_DATA[0])};
    BlockHeader header{block.GetHeader()};
    const std::chrono::seconds block_time{header.Timestamp()};

    // Attach clock to the chainstate manager
    chainman->SetClock(clock.get());

    // With time set 3h before the header, it appears >2h in the future: rejected
    clock.SetTime(block_time - std::chrono::hours{3});
    ...
    // Detach clock — restores real-time behavior
    chainman->SetClock(nullptr);
}
