...
BOOST_AUTO_TEST_CASE(btck_chainstate_manager_set_mock_time_tests)
{
    ...
    BOOST_CHECK_EXCEPTION(chainman->SetMockTime(std::chrono::seconds{-1}), std::runtime_error, HasReason("timestamp out of range"));
    BOOST_CHECK_EXCEPTION(chainman->SetMockTime(max_time + std::chrono::seconds{1}), std::runtime_error, HasReason("timestamp out of range"));
    ...
    chainman->SetMockTime(block_time - std::chrono::hours{3});
