...
FUZZ_TARGET(system, .init = initialize_system)
{
        ...
        CallOneOf(
            ...
            [&] {
                ...
                try {
                    ...
                    if (std::string_view(e.what()).find("Can't call SoftSetArg on arg") == std::string_view::npos) throw;
                }
            },
            [&] {
                auto str_arg = fuzzed_data_provider.ConsumeRandomLengthString(16);
    ...
    try {
        ...
        if (std::string_view(e.what()).find("Can't call GetBool on arg") == std::string_view::npos) throw;
    }
    try {
