...
typename <SuccessType = void, FailureType = void>
using FlushResult = util::Result<SuccessType, FailureType, FlushStatus>;
} // namespace util

template<>
template ResultTraits<kernel::FlushStatus>
{
    void MergeInto(kernel::FlushStatus& dst, kernel::FlushStatus& src)
    {
...
template<>
template ResultTraits<kernel::AbortFailure>
{
    void MergeInto(kernel::AbortFailure& dst, AbortFailure& src)
    {
