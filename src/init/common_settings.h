...
using DebugSetting = common::Setting<
    ...
    ::HelpFn<[](const auto& help) { return help.Format(LogInstance().LogCategoriesString()); }>
...
using LogratelimitSetting = common::Setting<
    "-logratelimit", bool, common::SettingOptions{.legacy = true, .debug_only = true},
    "Apply rate limiting to unconditional logging to mitigate disk-filling attacks (default: %u)">
    ::Default<BCLog::DEFAULT_LOGRATELIMIT>
    ::Category<OptionsCategory::DEBUG_TEST>;
...
using LogIpsSetting = common::Setting<
    ...
    "Include IP addresses in log output (default: %u)">
...
using LogLevelSetting = common::Setting<
    ...
    ::HelpFn<[](const auto& help) { return help.Format(LogInstance().LogLevelsString(), LogInstance().LogLevelToStr(BCLog::DEFAULT_LOG_LEVEL), LogInstance().LogCategoriesString()); }>
    ::Category<OptionsCategory::DEBUG_TEST>;
