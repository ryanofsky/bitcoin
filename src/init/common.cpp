...
{
    LogInstance().m_print_to_file = !DebuglogfileSettingDisabled::Get(args);
    LogInstance().m_file_path = AbsPathForConfigVal(args, DebuglogfileSetting::Get(args));
    LogInstance().m_print_to_console = PrinttoconsoleSetting::Get(args).value_or(!DaemonSetting::Get(args).value_or(false));
...
{
    if (LoglevelSetting::Get(args)) {
        for (const std::string& level_str : LoglevelSetting::Get(args).value_or(std::vector<std::string>{})) {
...
{
    if (DebugSetting::Get(args)) {
        // Special-case: if -debug=0/-nodebug is set, turn off debugging messages
        const std::vector<std::string> categories = DebugSetting::Get(args).value_or(std::vector<std::string>{});
