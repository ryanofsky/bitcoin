...
protected:
     ...
     * generated addresses for and would only be spendable because the keys were
     * familiar. (Before segwit activation it was also important to not treat
     * segwit outputs to arbitrary wallet keys as payments, because these would
     * be spendable by anyone without requiring any signing.)
     *
     ...
     * calls are all saved because they are all actually intended to receive
     * payments.
     *
     ...
     * wallet considers payments, and every output with a script it contains is
     * considered to belong to the wallet, regardless of whether the script is
     * solvable or signable. By contrast, the scripts in mapScripts are used to
     * determine solvability, and to restrict what outputs are considered
     * payments by the wallet. An output with a script in mapScripts, unlike
     * setWatchOnly, will not automatically be considered to belong to the
     * wallet if it can't be solved and signed for.
     */
