...
protected:
    ...
    //! Snapshot block hash if this chainstate was loaded from an assumeutxo
    //! snapshot, or null.
    ...
    //! chainstate were validated, false if the chain is based on an assumeutxo
    //! snapshot and the snapshot has not yet been validated.
    bool m_validated GUARDED_BY(::cs_main);
    ...
    //! most-work block.
    const CBlockIndex* m_target_block GUARDED_BY(::cs_main) {nullptr};
...
{
private:
    //! List of chainstates. The most-work chainstate is considered the "active"
    //! chainstate and any other chainstate is just used to validate historical
    //! blocks.
    //!
    //! In general, it is not safe delete Chainstate objects once they are added
    //! here because there is no mutex that can be locked to prevent Chainstate
    //! pointers from being used while they are deleted. (cs_main can't be used
    //! because it is released in the middle of Chainstate::ActivateBestChain so
    //! notifications can be processed). We could make deleting chainstate
    //! objects possible in the future by adding a new mutex with wider scope
    //! than cs_main whenever accessing chainstate objects. One way to do that
    //! could be to move `Chainstate::m_chainstate_mutex` mutex to
    //! ChainstateManager and reuse it.
...
public:
    ...
    Chainstate& MostWorkChainstate(bool require_validated = false) const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) {
    ...
    //! Return the active (most-work) chainstate.
    Chainstate& ActiveChainstate() const { LOCK(GetMutex()); return MostWorkChainstate(); }
    CChain& ActiveChain() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) { return ActiveChainstate().m_chain; }
    int ActiveHeight() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) { return ActiveChain().Height(); }
    CBlockIndex* ActiveTip() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) { return ActiveChain().Tip(); }

    // If the active chainstate is not validated, return the background chainstate that can be used to validate it.
    Chainstate* HistoricalChainstate() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) {
        return !MostWorkChainstate().m_validated ? &MostWorkChainstate(/*require_validated=*/true) : nullptr;
    }
