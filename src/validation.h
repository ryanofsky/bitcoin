...
public:
    ...
    //! Whether all blocks in the chain were validated, or the chainstate is
    //! based on an assumeutxo snapshot and the snapshot has not been validated.
    ...
    std::string ToString() const EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
...
public:
    ...
    //! Return chainstate targetting the most-most work valid block.
    Chainstate& MostWorkChainstate() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) {
        auto it = std::find_if(m_chainstates.begin(), m_chainstates.end(), [] (auto& cs) {
            return !cs->m_target_block &&
                   (cs->m_validity == ChainValidity::VALIDATED ||
                    cs->m_validity == ChainValidity::ASSUMED_VALID); });
        assert (it != m_chainstates.end());
        return *Assert(*it);
    }

    //! Return fully validated chainstate.
    ...
    //! If the active chainstate is not fully validated, return the background
    //! chainstate that can be used to download historical blocks and validate
    //! it.
    Chainstate* HistoricalChainstate() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex()) {
        return MostWorkChainstate().m_validity == ChainValidity::VALIDATED ? nullptr : &ValidatedChainstate();
    }

    //! Remove a chainstate.
    std::unique_ptr<Chainstate> RemoveChainstate(Chainstate& chainstate) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
    {
        auto it{std::find_if(m_chainstates.begin(), m_chainstates.end(), [&](auto& p){ return p.get() == &chainstate; })};
    ...
    std::optional<std::pair<const CBlockIndex*, const CBlockIndex*>> GetHistoricalBlockRange() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) const;

    util::Result<void> ActivateBestChains() LOCKS_EXCLUDED(::cs_main) const;
    void PruneAndFlush();
    ...
    //! are deleted. (cs_main can't be used because it is released in the middle
    //! of Chainstate::ActivateBestChain in order for notifications can be
    //! processed).
