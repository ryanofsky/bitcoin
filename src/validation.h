...
public:
    ...
    SnapshotCompletionResult MaybeCompleteSnapshotValidation(kernel::FlushResult<>& result) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
     ...
     * */
    kernel::FlushResult<kernel::InterruptResult> LoadExternalBlockFile(
        AutoFile& file_in,
     ...
     */
    kernel::FlushResult<> ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block) LOCKS_EXCLUDED(cs_main);
