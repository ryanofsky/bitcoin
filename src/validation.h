...
enum class Assumeutxo {
    ...
    VALIDATED,
    //! Block after an assumetxo snapshot have been validated but the snapshot itself has not been validated.
    UNVALIDATED,
    //! The assumetxo snapshot failed validation.
    INVALID,
};
...
public:
    ...
    //! `unvalidated_cs.m_assumeutxo` will be updated from UNVALIDATED to
    ...
    Chainstate* DetectAssumeutxoChainstate() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
