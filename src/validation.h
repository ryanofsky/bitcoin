...
[[nodiscard]] kernel::FlushResult<> PruneBlockFilesManual(Chainstate& active_chainstate, int nManualPruneHeight);
 ...
 */
[[nodiscard]] std::tuple<MempoolAcceptResult, kernel::FlushResult<>> AcceptToMemoryPool(Chainstate& active_chainstate, const CTransactionRef& tx,
...
=======
[[nodiscard]] std::tuple<PackageMempoolAcceptResult, kernel::FlushResult<>> ProcessNewPackage(Chainstate& active_chainstate, CTxMemPool& pool,
                                                   const Package& txns, bool test_accept, std::optional<CFeeRate> max_sane_feerate)
...
public:
    ...
    [[nodiscard]] kernel::FlushResult<> ResizeCoinsCaches(size_t coinstip_size, size_t coinsdb_size)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
     ...
     */
    [[nodiscard]] kernel::FlushResult<> FlushStateToDisk(
    ...
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    [[nodiscard]] kernel::FlushResult<> ConnectBlock(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex,
...
private:
    [[nodiscard]] kernel::FlushResult<> ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
    [[nodiscard]] kernel::FlushResult<> ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
...
public:
    ...
    [[nodiscard]] SnapshotCompletionResult MaybeCompleteSnapshotValidation(kernel::FlushResult<>& result) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
     ...
     * */
    [[nodiscard]] kernel::FlushResult<kernel::InterruptResult> LoadExternalBlockFile(
        AutoFile& file_in,
     ...
     */
    [[nodiscard]] bool ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block, kernel::FlushResult<>& result) LOCKS_EXCLUDED(cs_main);
     ...
     */
    [[nodiscard]] bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, kernel::FlushResult<>& result, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
     ...
     * @param[in]  test_accept     When true, run validation checks but don't submit to mempool.
     */
    [[nodiscard]] std::tuple<MempoolAcceptResult, kernel::FlushResult<>> ProcessTransaction(const CTransactionRef& tx, bool test_accept=false)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    //! Load the block tree and coins database from disk, initializing state if we're running with -reindex
    [[nodiscard]] util::Result<kernel::InterruptResult> LoadBlockIndex() EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    ...
    [[nodiscard]] util::Result<void> ValidatedSnapshotCleanup() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
