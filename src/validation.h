...
public:
    ...
    //! Try to validate an assumed-valid chainstate which is based on a UTXO
    //! snapshot by using a fully-validated historical chainstate targeted at
    //! the snapshot block. When the target block is reached, the UTXO hash is
    //! computed and saved to `validated_cs.m_target_utxohash`, and
    //! `assumed_valid_cs.m_validity` will be updated from ASSUMED_VALID to
    //! either VALIDATED or INVALID depending on whether the hash matches. The
    //! INVALID case should never happen because the software should refused to
    //! load unrecognized snapshots, but if it does happen it is a fatal error.
    SnapshotCompletionResult MaybeValidateSnapshot(Chainstate& validated_cs, Chainstate& assumed_valid_cs) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    ...
    Chainstate* DetectAssumedValidChainstate() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    ...
    bool ValidatedSnapshotCleanup(Chainstate& validated_cs, Chainstate& assumed_valid_cs) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
