...
||||||| parent of 8f1f60f9343 (refactor: Use util::Result class in wallet::MakeDatabase)
    std::unique_ptr<WalletDatabase> database = MakeDatabase(path, options, status, error);
=======
    std::unique_ptr<WalletDatabase> database = ResultExtract(MakeDatabase(path, options), &status, &error);
>>>>>>> 8f1f60f9343 (refactor: Use util::Result class in wallet::MakeDatabase)
||||||| parent of a85d65598e3 (refactor: Use util::Result class in wallet/wallettool)
    DatabaseStatus status;
    bilingual_str error;
    std::unique_ptr<WalletDatabase> database = ResultExtract(MakeDatabase(path, options), &status, &error);
=======
    auto database{MakeDatabase(path, options)};
>>>>>>> a85d65598e3 (refactor: Use util::Result class in wallet/wallettool)
    if (!database) {
