...
{
    if (FormatSetting::Get(args) && command != "createfromdump") {
        tfm::format(std::cerr, "The -format option can only be used with the \"createfromdump\" command.\n");
        return false;
    }
    if (DumpfileSetting::Get(args) && command != "dump" && command != "createfromdump") {
        tfm::format(std::cerr, "The -dumpfile option can only be used with the \"dump\" and \"createfromdump\" commands.\n");
        return false;
    }
    if (DescriptorsSetting::Get(args) && command != "create") {
        tfm::format(std::cerr, "The -descriptors option can only be used with the 'create' command.\n");
        return false;
    }
    if (LegacySetting::Get(args) && command != "create") {
        tfm::format(std::cerr, "The -legacy option can only be used with the 'create' command.\n");
        return false;
    }
    if (command == "create" && !WalletSetting::Get(args)) {
        tfm::format(std::cerr, "Wallet name must be provided when creating a new wallet.\n");
        return false;
    }
    const std::string name = WalletSetting::Get(args).value_or("");
    ...
    if (command == "create") {
        ...
        bool make_legacy = LegacySetting::Get(args).value_or(false);
        // If neither -legacy nor -descriptors is set, default to true. If -descriptors is set, use its value.
        bool make_descriptors = (!DescriptorsSetting::Get(args) && !LegacySetting::Get(args)) || (DescriptorsSetting::Get(args) && DescriptorsSetting::Get(args).value_or(true));
