...
void SQLiteDatabase::Open(const char* mode)
{
    ...
    if (m_db == nullptr) {
        ...
        if (create) {
            ...
            // Ensure the application ID matches our network magic
            sqlite3_stmt* app_id_stmt{nullptr};
            ret = sqlite3_prepare_v2(db, "PRAGMA application_id", -1, &app_id_stmt, nullptr);
            if (ret != SQLITE_OK) {
                sqlite3_finalize(app_id_stmt);
                sqlite3_close(db);
                throw std::runtime_error(strprintf("SQLiteDatabase: Failed to prepare the statement to fetch the application id: %s", sqlite3_errstr(ret)));
            }
            ret = sqlite3_step(app_id_stmt);
            if (ret != SQLITE_ROW) {
                sqlite3_finalize(app_id_stmt);
                sqlite3_close(db);
                throw std::runtime_error(strprintf("SQLiteDatabase: Failed to fetch the application id: %s", sqlite3_errstr(ret)));
            }
            uint32_t app_id = (uint32_t)sqlite3_column_int(app_id_stmt, 0);
            sqlite3_finalize(app_id_stmt);
            uint32_t net_magic = ReadBE32(Params().MessageStart());
            ...
            // Ensure our schema version
            sqlite3_stmt* user_ver_stmt{nullptr};
            ret = sqlite3_prepare_v2(db, "PRAGMA user_version", -1, &user_ver_stmt, nullptr);
            if (ret != SQLITE_OK) {
                sqlite3_finalize(user_ver_stmt);
                sqlite3_close(db);
                throw std::runtime_error(strprintf("SQLiteDatabase: Failed to prepare the statement to fetch sqlite wallet schema version: %s", sqlite3_errstr(ret)));
            }
            ret = sqlite3_step(user_ver_stmt);
            if (ret != SQLITE_ROW) {
                sqlite3_finalize(user_ver_stmt);
                sqlite3_close(db);
                throw std::runtime_error(strprintf("SQLiteDatabase: Failed to fetch sqlite wallet schema version: %s", sqlite3_errstr(ret)));
            }
            int32_t user_ver = sqlite3_column_int(user_ver_stmt, 0);
            sqlite3_finalize(user_ver_stmt);
            if (user_ver != WALLET_SCHEMA_VERSION) {
