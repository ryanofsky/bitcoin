...
=======
    case DBErrors::NEED_REWRITE:
        result.messages().errors.push_back(strprintf(_("Wallet needed to be rewritten: restart %s to complete"), CLIENT_NAME));
        break;
>>>>>>> b5c51c10e85 (refactor: Use util::Result class in wallet/wallet)
...
=======
    if (const auto arg{args.GetArg("-paytxfee")}) {
        ...
        std::optional<CAmount> pay_tx_fee = ParseMoney(*arg);
        if (!pay_tx_fee) {
            result.update(Error{AmountErrMsg("paytxfee", *arg)});
            return result;
        } else if (pay_tx_fee.value() > HIGH_TX_FEE_PER_KB) {
        ...
        if (chain && wallet->m_pay_tx_fee < chain->relayMinFee()) {
            result.update(Error{strprintf(_("Invalid amount for %s=<amount>: '%s' (must be at least %s)"),
                "-paytxfee", *arg, chain->relayMinFee().ToString())});
            return result;
        }
    }
