...
bool CWallet::Open(std::shared_ptr<CWallet> wallet, const DatabaseOptions& options, DatabaseStatus& status, bilingual_str& error, std::vector<bilingual_str>& warnings) {
    status = DatabaseStatus::FAILED_LOAD;
    try {
        fs::path path;
        if (!CWallet::Path(wallet->m_name, path, error)) return false;
        std::unique_ptr<WalletDatabase> db = MakeDatabase(path, options, status, error);
        if (!db || !CWallet::Create(wallet, std::move(db), options.create_passphrase, options.create_flags, status, error, warnings)) return false;
...
bool CWallet::Create(std::shared_ptr<CWallet> wallet, std::unique_ptr<WalletDatabase> database, const SecureString& passphrase, uint64_t wallet_creation_flags, DatabaseStatus& status, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    // Make the wallet
    if (!CWallet::Load(wallet, std::move(database), wallet_creation_flags, error, warnings)) {
...
bool CWallet::Path(const std::string& wallet_name, fs::path& wallet_path, bilingual_str& error_string)
{
    ...
    wallet_path = fs::absolute(wallet_name, GetWalletDir());
    ...
    if (!(path_type == fs::file_not_found || path_type == fs::directory_file ||
        ...
        error_string = Untranslated(strprintf(
              ...
              wallet_name, GetWalletDir()));
        return false;
    }
    return true;
}

std::shared_ptr<CWallet> CWallet::Load(std::shared_ptr<CWallet> walletInstance, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (gArgs.GetBoolArg("-zapwallettxes", false)) {
        ...
        std::unique_ptr<CWallet> tempWallet = MakeUnique<CWallet>(&chain, walletInstance->m_name, std::move(database));
        DBErrors nZapWalletRet = tempWallet->ZapWalletTx(vWtx);
        if (nZapWalletRet != DBErrors::LOAD_OK) {
            error = strprintf(_("Error loading %s: Wallet corrupted"), walletFile);
            return nullptr;
        }
        database = std::move(tempWallet->database);
    }
    ...
    bool fFirstRun = true;
    walletInstance->database = std::move(database);
