...
>>>>>>> Drop Chain::requestMempoolTransactions method
    ...
    wallet->m_chain_notifications_handler = wallet->chain().handleNotifications(
        wallet, scan_fn, mempool_fn, best_block_locator.get_ptr(), time_first_key.value_or(0),
        FoundBlock().hash(wallet->m_last_block_processed).height(wallet->m_last_block_processed_height).locator(last_block_processed_locator),
        missing_block_data);
