...
std::shared_ptr<CWallet> CWallet::Create(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (MintxfeeSetting::Get(args)) {
        std::optional<CAmount> min_tx_fee = ParseMoney(MintxfeeSetting::Get(args).value_or(""));
        if (!min_tx_fee) {
            error = AmountErrMsg("mintxfee", MintxfeeSetting::Get(args).value_or(""));
            return nullptr;
    ...
    if (MaxapsfeeSetting::Get(args)) {
        const std::string max_aps_fee{MaxapsfeeSetting::Get(args).value_or("")};
        if (max_aps_fee == "-1") {
    ...
    if (FallbackfeeSetting::Get(args)) {
        std::optional<CAmount> fallback_fee = ParseMoney(FallbackfeeSetting::Get(args).value_or(""));
        if (!fallback_fee) {
            error = strprintf(_("Invalid amount for %s=<amount>: '%s'"), "-fallbackfee", FallbackfeeSetting::Get(args).value_or(""));
            return nullptr;
    ...
    if (DiscardfeeSetting::Get(args)) {
        std::optional<CAmount> discard_fee = ParseMoney(DiscardfeeSetting::Get(args).value_or(""));
        if (!discard_fee) {
            error = strprintf(_("Invalid amount for %s=<amount>: '%s'"), "-discardfee", DiscardfeeSetting::Get(args).value_or(""));
            return nullptr;
    ...
    if (PaytxfeeSetting::Get(args)) {
        std::optional<CAmount> pay_tx_fee = ParseMoney(PaytxfeeSetting::Get(args).value_or(""));
        if (!pay_tx_fee) {
            error = AmountErrMsg("paytxfee", PaytxfeeSetting::Get(args).value_or(""));
            return nullptr;
        ...
        if (chain && walletInstance->m_pay_tx_fee < chain->relayMinFee()) {
            error = strprintf(_("Invalid amount for %s=<amount>: '%s' (must be at least %s)"),
                "-paytxfee", PaytxfeeSetting::Get(args).value_or(""), chain->relayMinFee().ToString());
            return nullptr;
        }
    }

    if (MaxtxfeeSetting::Get(args)) {
        std::optional<CAmount> max_fee = ParseMoney(MaxtxfeeSetting::Get(args).value_or(""));
        if (!max_fee) {
            error = AmountErrMsg("maxtxfee", MaxtxfeeSetting::Get(args).value_or(""));
            return nullptr;
        ...
        if (chain && CFeeRate{max_fee.value(), 1000} < chain->relayMinFee()) {
            error = strprintf(_("Invalid amount for %s=<amount>: '%s' (must be at least the minrelay fee of %s to prevent stuck transactions)"),
                "-maxtxfee", MaxtxfeeSetting::Get(args).value_or(""), chain->relayMinFee().ToString());
            return nullptr;
        }
    ...
    if (ConsolidatefeerateSetting::Get(args)) {
        if (std::optional<CAmount> consolidate_feerate = ParseMoney(ConsolidatefeerateSetting::Get(args).value_or(""))) {
            walletInstance->m_consolidate_feerate = CFeeRate(*consolidate_feerate);
        } else {
            error = AmountErrMsg("consolidatefeerate", ConsolidatefeerateSetting::Get(args).value_or(""));
            return nullptr;
        }
    }
