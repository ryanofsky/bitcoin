...
util::ResultPtr<std::shared_ptr<CWallet>, DatabaseStatus> LoadWalletInternal(WalletContext& context, const std::string& name, std::optional<bool> load_on_start, const DatabaseOptions& options)
{
    ...
    try {
        ...
        if (!database) {
            result.Update({util::Error{Untranslated("Wallet file verification failed.")}, database.GetFailure()});
            return result;
        }
        ...
        if (!wallet) {
            result.Update({util::Error{Untranslated("Wallet loading failed.")}, DatabaseStatus::FAILED_LOAD});
            return result;
        }
...
util::ResultPtr<std::shared_ptr<CWallet>, DatabaseStatus> CreateWallet(WalletContext& context, const std::string& name, std::optional<bool> load_on_start, DatabaseOptions& options)
{
    ...
    auto database{MakeWalletDatabase(name, options) >> result};
    if (!database) {
        result.Update({util::Error{Untranslated("Wallet file verification failed.")}, DatabaseStatus::FAILED_VERIFY});
        return result;
    }
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(const std::string& wallet_name, const SecureString& passphrase, WalletContext& context)
{
    ...
    auto database{MakeWalletDatabase(wallet_name, options) >> result};
    if (!database) {
        result.Update(util::Error{Untranslated("Wallet file verification failed.")});
        return result;
    }
    ...
    if (!local_wallet) {
        result.Update(util::Error{Untranslated("Wallet loading failed.") + Untranslated(" ")});
        return result;
    }
