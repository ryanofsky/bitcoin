...
void CWallet::GetKeyBirthTimes(interfaces::Chain::Lock& locked_chain, std::map<CKeyID, int64_t>& mapKeyBirth) const {
    ...
    max_confirm.block_height = m_last_block_processed_height > 144 ? m_last_block_processed_height - 144 : 0; // the tip can be reorganized; use a 144-block safety margin
    max_confirm.hashBlock = chain().findAncestorByHeight(m_last_block_processed, max_confirm.block_height);
