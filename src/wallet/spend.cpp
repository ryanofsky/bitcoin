...
void AvailableCoins(const CWallet& wallet, std::vector<COutput>& vCoins, bool fOnlySafe, const CCoinControl* coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount)
{
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CoinSelectionParams& coin_selection_params, bool& bnb_used)
{
    ...
    if (coin_selection_params.use_bnb) {
        ...
        CFeeRate long_term_feerate = GetMinimumFeeRate(wallet, temp, &feeCalc);
        ...
        std::vector<OutputGroup> groups = GroupOutputs(wallet, coins, !coin_selection_params.m_avoid_partial_spends, effective_feerate, long_term_feerate, eligibility_filter, true /* positive_only */);

        // Calculate cost of change
        CAmount cost_of_change = GetDiscardRate(wallet).GetFee(coin_selection_params.change_spend_size) + coin_selection_params.effective_fee.GetFee(coin_selection_params.change_output_size);
        ...
        std::vector<OutputGroup> groups = GroupOutputs(wallet, coins, !coin_selection_params.m_avoid_partial_spends, CFeeRate(0), CFeeRate(0), eligibility_filter, false /* positive_only */);
...
bool SelectCoins(const CWallet& wallet, const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used)
{
    ...
    bool res = value_to_select <= 0 ||
        SelectCoinsMinConf(wallet, value_to_select, CoinEligibilityFilter(1, 6, 0), vCoins, setCoinsRet, nValueRet, coin_selection_params, bnb_used) ||
        SelectCoinsMinConf(wallet, value_to_select, CoinEligibilityFilter(1, 1, 0), vCoins, setCoinsRet, nValueRet, coin_selection_params, bnb_used) ||
        (wallet.m_spend_zero_conf_change && SelectCoinsMinConf(wallet, value_to_select, CoinEligibilityFilter(0, 1, 2), vCoins, setCoinsRet, nValueRet, coin_selection_params, bnb_used)) ||
        (wallet.m_spend_zero_conf_change && SelectCoinsMinConf(wallet, value_to_select, CoinEligibilityFilter(0, 1, std::min((size_t)4, max_ancestors/3), std::min((size_t)4, max_descendants/3)), vCoins, setCoinsRet, nValueRet, coin_selection_params, bnb_used)) ||
        (wallet.m_spend_zero_conf_change && SelectCoinsMinConf(wallet, value_to_select, CoinEligibilityFilter(0, 1, max_ancestors/2, max_descendants/2), vCoins, setCoinsRet, nValueRet, coin_selection_params, bnb_used)) ||
        (wallet.m_spend_zero_conf_change && SelectCoinsMinConf(wallet, value_to_select, CoinEligibilityFilter(0, 1, max_ancestors-1, max_descendants-1, true /* include_partial_groups */), vCoins, setCoinsRet, nValueRet, coin_selection_params, bnb_used)) ||
        (wallet.m_spend_zero_conf_change && !fRejectLongChains && SelectCoinsMinConf(wallet, value_to_select, CoinEligibilityFilter(0, 1, std::numeric_limits<uint64_t>::max(), std::numeric_limits<uint64_t>::max(), true /* include_partial_groups */), vCoins, setCoinsRet, nValueRet, coin_selection_params, bnb_used));
        ...
        bool sign)
{
    ...
    int nBytes;
    {
        ...
        txNew.nLockTime = GetLocktimeForNewTransaction(wallet.chain(), wallet.GetLastBlockHash(), wallet.GetLastBlockHeight());
        {
            ...
            AvailableCoins(wallet, vAvailableCoins, true, &coin_control, 1, MAX_MONEY, MAX_MONEY, 0);
            ...
            CFeeRate discard_rate = GetDiscardRate(wallet);

            // Get the fee rate to use effective values in coin selection
            CFeeRate nFeeRateNeeded = GetMinimumFeeRate(wallet, coin_control, &feeCalc);
            ...
            while (true)
            {
                ...
                nFeeNeeded = GetMinimumFee(wallet, nBytes, coin_control, &feeCalc);
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && pick_new_inputs) {
                        ...
                        CAmount fee_needed_with_change = GetMinimumFee(wallet, tx_size_with_change, coin_control, nullptr);
