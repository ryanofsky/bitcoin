...
bool WalletInit::ParameterInteraction() const
{
    ...
    if (DisablewalletSetting::Get(gArgs)) {
        for (const std::string& wallet : WalletSetting::Get(gArgs).value_or(std::vector<std::string>{})) {
    ...
    if (BlocksonlySetting::Get(gArgs).value_or(DEFAULT_BLOCKSONLY) && gArgs.SoftSetBoolArg("-walletbroadcast", false)) {
