...
public:
    ...
    void disconnectIncoming() override
    {
        ...
        std::vector<mp::Connection::Tracker> disconnected;
        m_loop->sync([&] {
            for (mp::Connection& c : m_loop->incomingConnections()) {
                if (&c != m_parent_connection) {
                    disconnected.push_back(c.tracker());
                    c.disconnect();
                }
            }
        });
        ...
        size_t pending{0};
        for (const auto& connection : disconnected) pending += connection->pendingServerObjects();
        if (pending > 0) {
        ...
        for (const auto& tracker : disconnected) tracker->waitDrained();
