...
public:
    ...
    {
        assert(!m_loop);
        mp::g_thread_context.thread_name = mp::ThreadName(m_loop->m_exe_name);
    ...
    std::array<mp::Stream, 2> makeSocketPair() override
    {
        startLoop();
        auto pipe = m_loop->m_io_context.provider->newTwoWayPipe();
        return {std::move(pipe.ends[0]), std::move(pipe.ends[1])};
    }
