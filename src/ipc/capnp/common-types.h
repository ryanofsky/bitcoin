...
//! Overload CustomBuildField and CustomReadField to serialize util::Expected
//! return values as common.capnp Expected union structs
template <typename T, typename E, typename Value, typename Output>
void CustomBuildField(TypeList<util::Expected<T, E>>, Priority<1>, InvokeContext& invoke_context, Value&& expected, Output&& output)
{
    auto result = output.init();
    if (expected) {
        BuildField(TypeList<T>(), invoke_context, Make<ValueField>(result.initValue()), *expected);
    } else {
        BuildField(TypeList<E>(), invoke_context, Make<ValueField>(result.initError()), expected.error());
    }
}

template <typename T, typename E, typename Input, typename ReadDest>
decltype(auto) CustomReadField(TypeList<util::Expected<T, E>>, Priority<1>, InvokeContext& invoke_context, Input&& input, ReadDest&& read_dest)
{
    auto result = input.get();
    if (result.hasError()) {
        return ReadField(TypeList<E>(), invoke_context, Make<ValueField>(result.getError()),
            ReadDestEmplace(TypeList<E>(), [&](auto&&... args) -> const util::Expected<T, E>& {
                return read_dest.construct(util::Unexpected{std::forward<decltype(args)>(args)...});
            }));
    } else {
        return ReadField(TypeList<T>(), invoke_context, Make<ValueField>(result.getValue()),
            ReadDestEmplace(TypeList<T>(), [&](auto&&... args) -> const util::Expected<T, E>& {
                return read_dest.construct(std::forward<decltype(args)>(args)...);
            }));
    }
}
