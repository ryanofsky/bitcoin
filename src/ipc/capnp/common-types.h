...
namespace ipc {
namespace capnp {
//! Use SFINAE to define Serializeable<T>::value which is true if type T has a
...
//! Use SFINAE to define Unserializeable<T>::value which is true if type T has
//! a Unserialize(stream) method, false otherwise.
template <typename T>
...
CustomReadField(TypeList<LocalType>, Priority<1>, InvokeContext& invoke_context, Input&& input, ReadDest&& read_dest,
                typename std::enable_if_t<ipc::capnp::Unserializable<LocalType>::value>* enable = nullptr)
{
