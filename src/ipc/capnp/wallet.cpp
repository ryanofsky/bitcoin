...
// Longer term solution would be for CleanupList to use C++23 std::move_only_function
template<class F>
...
void ProxyServerMethodTraits<ipc::capnp::messages::ChainClient::StartParams>::invoke(WalletLoaderContext& context)
{
    if (!context.proxy_server.m_scheduler) {
        ...
        context.proxy_server.m_context.cleanup_fns.emplace_back(make_shared_function([s=std::move(scheduler)]() mutable { s.reset(); }));
    }
                       ...
                       CTxDestination& dest)
{
    if (reader.hasPubKey()) {
        ...
        dest = WitnessV1Taproot{XOnlyPubKey{std::span{data.begin(), data.size()}}};
        ...
        ReadField(TypeList<WitnessUnknown>(), invoke_context, Make<ValueField>(reader.getWitnessUnknown()),
                  ReadDestUpdate(std::get<WitnessUnknown>(dest)));
    }
}
