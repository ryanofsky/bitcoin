...
interface Wallet $Proxy.wrap("interfaces::Wallet") {
    ...
    # TODO: Should drop nSigned field from input params, and only keep output
    # nSigned field. Only reason it is listed in inputs is because
    # libmultiprocess requires parameters to be listed in IN > INOUT > OUT
    # order, so making in an OUT param would require rearraning parameters
    # placing nSigned after psbtx and it would be more work to reorder
    # parameters.
    fillPSBT @39 (context :Proxy.Context, options :Common.PSBTFillOptions, nSigned :UInt64, wantNSigned :Bool, psbtx :Data) -> (nSigned: UInt64, psbtx :Data, complete :Bool, result :Int32, hasResult: Bool);
...
struct WalletBalances $Proxy.wrap("interfaces::WalletBalances") {
    ...
    usedBalance @3 :Int64 $Proxy.name("used_balance");
    nonmempoolBalance @4 :Int64 $Proxy.name("nonmempool_balance");
}
