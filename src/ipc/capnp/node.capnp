...
struct NodeStateStats $Proxy.wrap("CNodeStateStats") {
    ...
    relayTxs @5 :Bool $Proxy.name("m_relay_txs");
    invToSend @6 :Int32 $Proxy.name("m_inv_to_send");
    lastInvSeq @7 :UInt64 $Proxy.name("m_last_inv_seq");
    feeFilterReceived @8 :Int64 $Proxy.name("m_fee_filter_received");
    addressesProcessed @9 :UInt64 $Proxy.name("m_addr_processed");
    addressesRateLimited @10 :UInt64 $Proxy.name("m_addr_rate_limited");
    addressRelayEnabled @11 :Bool $Proxy.name("m_addr_relay_enabled");
    theirServices @12 :UInt64 $Proxy.name("their_services");
    presyncHeight @13 :Int64 $Proxy.name("presync_height");
