               ...
               interfaces::FoundBlock& found_block)
{
    ...
    if (reader.getWantData()) builder.setData(ipc::capnp::ToArray(ipc::capnp::Serialize(TX_WITH_WITNESS(data))));
                       ...
                       const interfaces::FoundBlock& dest)
{
    ...
    if (dest.m_data) *dest.m_data = ipc::capnp::Unserialize<CBlock>(reader.getData(), TX_WITH_WITNESS);
    dest.found = true;
}
                        ...
                        ipc::capnp::messages::BlockInfo::Builder&& builder)
{
    ...
    if (block.data) builder.setData(ipc::capnp::ToArray(ipc::capnp::Serialize(TX_WITH_WITNESS(*block.data))));
                        ...
                        std::function<void(const interfaces::BlockInfo&)>&& fn)
{
    ...
    if (reader.hasData()) {
        data.emplace(ipc::capnp::Unserialize<CBlock>(reader.getData(), TX_WITH_WITNESS));
        block.data = &*data;
    }
