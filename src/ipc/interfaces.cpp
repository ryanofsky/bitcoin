...
public:
    ...
    {
        auto pair{m_protocol->makeSocketPair()};
        mp::ProcessId pid{m_process->spawn(mp::StreamSocketId(pair[0]), new_exe_name, m_process_argv0)};
        LogDebug(::BCLog::IPC, "Process %s pid %i launched\n", new_exe_name, pid);
        auto init = m_protocol->connect(std::move(pair[1]));
