...
Connection::~Connection()
{
    ...
    //
    // Sending pending data is important if the connection is a
    // socketpair because when one side of the socketpair is closed, the other side
    // doesn't seem to receive any onDisconnect event. So it is important for the
    // other side to instead receive Cap'n Proto "release" messages (see `struct
    // Release` in capnp/rpc.capnp) from local Client objects being being destroyed
    // so the remote side can free resources and shut down cleanly.
...
//! support for synchronous IO. It has FdOutputStream wrapping unix file
//! descriptors and calling write() internally, it has HandleOutStream wrapping
//! windows HANDLE values and calling WriteFile internally. This class just
//! provides analagous functionality wrapping SOCKET values and calling send()
//! internally.
