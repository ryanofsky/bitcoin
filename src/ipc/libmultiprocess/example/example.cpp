...
static auto Spawn(mp::EventLoop& loop, const std::string& process_argv0, const std::string& new_exe_name)
{
    ...
    mp::ProcessId pid{mp::SpawnProcess(mp::StreamSocketId(pipe.ends[0]), [&](mp::ConnectInfo info) -> std::vector<std::string> {
        fs::path path = process_argv0;
        path.remove_filename();
        path.append(new_exe_name);
        return {path.string(), std::move(info)};
    })};
    return std::make_tuple(mp::ConnectStream<InitInterface>(loop, std::move(pipe.ends[1])), pid);
}
