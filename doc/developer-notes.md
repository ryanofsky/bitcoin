...
classes like [`interfaces::Handler`](../src/interfaces/handler.h)
...
- Interface classes should be abstract and have methods that are [pure
  ...
  especially so one implementation can execute functionality in the local
...
- Interface method definitions should wrap existing functionality instead of
  ...
  [`src/interfaces/`](../src/interfaces/) rather than being implemented there,
  so it can be more modular and amenable to unit testing.

- Interface method parameter and return types should either be serializable or
  point to other interface classes. Interface methods shouldn't pass references
  to objects that can't be serialized or accessed from another process.
...
- For consistency and friendliness to code generation tools, interface method
 ...
 ```c++
  ...
  [`src/interfaces/`](../src/interfaces/), though it did come up once for
  discussion in [#14635](https://github.com/bitcoin/bitcoin/pull/14635).
