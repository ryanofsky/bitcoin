...
The choice to use an RPC framework at all instead of a custom protocol was neccesitated by the size of Bitcoin Core internal interfaces which consist of around 150 methods that pass around complex data structures and are called in complicated ways (in parallel, in blocking calls, and birectionally in nested and stored callbacks). Writing a custom protocol to wrap this complicated interface would be too much work, akin to writing a new RPC framework.
